...
#define PARTITION_ROUTER_RESULT_FULL  -1

typedef enum QulificationType
{
    QULIFICATION_TYPE_LS,
    QULIFICATION_TYPE_LE,
    QULIFICATION_TYPE_EQUAL,
    QULIFICATION_TYPE_GE,
    QULIFICATION_TYPE_GT
}QulificationType;

typedef struct AddDropPartitions
{
    NodeTag     type;
    int         nparts;
}AddDropPartitions;

typedef struct ModifyPartStartValue
{
    NodeTag     type;
    Node        *startvalue;
} ModifyPartStartValue;

/* partition by clause for interval partition */
...
typedef struct RangeTblEntry
{
    ...
    Node        *partvalue;     /* value to locate partition child tables */
#endif
...
typedef struct CopyStmt
{
    ...
    int         ncolumns;       /* number of insert columns*/
    int         ndatarows;      /* number of insert values */
#endif
} CopyStmt;
...
typedef struct CreateExtensionStmt
{
    ...
    char       *extname;
    bool        if_not_exists;    /* just do nothing if it already exists? */
    int            action;            /* prepare, execute or create */
    List       *options;        /* List of DefElem nodes */
} CreateExtensionStmt;
...
#ifdef _SHARDING_
typedef enum TruncateShardOption
{
    TRUNSHARDOPT_FREESTORAGE = 1 << 0
}TruncateShardOption;

typedef struct VacuumShardStmt
{
    NodeTag        type;
    int         options;
    RangeVar   *relation;
...
typedef enum AlterGroupType
{
    AG_SetDefault                /* set to default */
} AlterGroupType;


typedef struct AlterGroupCmd    /* one subcommand of an ALTER NODE GROUP */
{
    NodeTag            type;
    AlterGroupType  subtype;        /* Type of group alteration to apply */
} AlterGroupCmd;
...
typedef struct CreatePublicationStmt
{
    ...
    List       *shards;         /* Optional list of shards to add */
#endif
...
#ifdef _MIGRATE_
#define MOVE_DATA_STRATEGY_NODE     1
#define MOVE_DATA_STRATEGY_SHARD    2
#define MOVE_DATA_STRATEGY_AT        3

typedef struct MoveDataStmt
{
    NodeTag          type;
...
typedef struct CreateShardStmt
{
    NodeTag        type;
    bool        isnull;
    bool        isExtended;        /* extended or not */
    int            num_of_node;
    int            idx_of_node;
    List        *members;        /* groups */
}CreateShardStmt;

typedef struct DropShardStmt
{
    NodeTag        type;
    char        *group_name;        /* group name */
}DropShardStmt;

typedef struct CleanShardingStmt
{
    NodeTag        type;
    char        *group_name;
    char        *dn_from;
...
typedef struct ExchangeIndexName
{
    NodeTag        type;
    RangeVar    *oldIndexRelation;
    RangeVar    *newIndexRelation;
    Oid            oldIndexId;
    Oid            newIndexId;
}ExchangeIndexName;

#ifdef __COLD_HOT__
typedef struct CreateKeyValuesStmt
{
    NodeTag        type;
...
typedef struct CheckOverLapStmt
{
    NodeTag        type;
    List       *first;        /* list of Value strings */
    List       *second;        /* list of Value strings */    
} CheckOverLapStmt;
...
{
    AuditType_Statement = (1 << 1),
    AuditType_User        = (1 << 2),
    AuditType_Object    = (1 << 3),
} AuditType;

typedef enum AuditMode
{
    AuditMode_All         = 'a',
    AuditMode_Success    = 's',
    AuditMode_Fail        = 'f',
...
typedef struct AuditStmt
{
    NodeTag        type;
    bool        audit_ison;            /* audit or no audit */
    AuditType    audit_type;            /* audit sql, user or object */
    AuditMode    audit_mode;            /* successful or not successfull */
    List         *action_list;        /* list of action id of AuditSQLAction */
    List        *user_list;            /* user list */
    List        *object_name;        /* object_name, list of strVal */
    ObjectType    object_type;        /* object_type, see definition in ObjectType */
}AuditStmt;

typedef enum CleanAuditType
{
    CleanAuditType_All            = (1 << 1),        /* Clean All Audit */
    CleanAuditType_Statement     = (1 << 2),        /* Clean Statement Audit */
    CleanAuditType_User            = (1 << 3),        /* Clean User Audit */
    CleanAuditType_ByUser        = (1 << 4),        /* Clean User Audit By xxx */
    CleanAuditType_Object        = (1 << 5),        /* Clean Object Audit */
    CleanAuditType_OnObject        = (1 << 6),        /* Clean Object Audit On xxx */
    CleanAuditType_OnDefault    = (1 << 7),        /* Clean Object Audit On Default */
    CleanAuditType_Unknown        = (1 << 8),        /* Clean Unknown Audit */
} CleanAuditType;

typedef struct CleanAuditStmt        /* clean audit*/
{
    NodeTag            type;
    CleanAuditType    clean_type;            /* audit sql, user or object */
    List            *user_list;            /* user list */
    List            *object_name;        /* object_name, list of strVal */
    ObjectType        object_type;        /* object_type, see definition in ObjectType */
}CleanAuditStmt;
