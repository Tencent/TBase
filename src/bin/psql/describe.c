...
{// #lizard forgives
        ...
        tableinfo.relkind == RELKIND_PARTITIONED_TABLE)
    {
        ...
        if (verbose && (tableinfo.relkind == 'r' || tableinfo.relkind == 'p'))
        {
            ...
            printfPQExpBuffer(&buf,
                    "SELECT "
                    "CASE pclocatortype WHEN '%c' THEN 'ROUND ROBIN' WHEN '%c' THEN 'REPLICATION' WHEN '%c' THEN 'HASH' WHEN '%c' THEN 'SHARD' WHEN '%c' THEN 'MODULO' END as distype,    "
                    "(SELECT  attname where  a.attrelid = c.pcrelid and a.attnum = c.pcattnum) as discolumn, "
                    "(SELECT  attname from pg_catalog.pgxc_class x,  pg_catalog.pg_attribute a1 where  a1.attrelid = %s and a1.attnum = x.psecondattnum and x.pcrelid = %s) as disseccolumn, "
                    "CASE array_length(nodeoids, 1) WHEN nc.dn_cn THEN 'ALL DATANODES' ELSE array_to_string(ARRAY( SELECT node_name FROM pg_catalog.pgxc_node WHERE oid in (SELECT unnest(nodeoids) FROM pg_catalog.pgxc_class WHERE pcrelid = '%s') ), ', ') END as loc_nodes, "
                    "CASE array_length(coldnodeoids, 1) WHEN nc.dn_cn THEN 'ALL DATANODES' ELSE array_to_string(ARRAY( SELECT node_name FROM pg_catalog.pgxc_node WHERE oid in (SELECT unnest(coldnodeoids) FROM pg_catalog.pgxc_class WHERE pcrelid = '%s') ), ', ') END as cold_nodes "
                    "FROM pg_catalog.pg_attribute a right join pg_catalog.pgxc_class c on a.attrelid = c.pcrelid and a.attnum = c.pcattnum, (SELECT count(*) AS dn_cn FROM pg_catalog.pgxc_node WHERE node_type = 'D') as nc WHERE pcrelid = '%s' "
                    , LOCATOR_TYPE_RROBIN
                        ...
                        "SELECT CASE pclocatortype \n"
                            ...
                            ", CASE array_length(nodeoids, 1) \n"
                                ...
                                "ELSE array_to_string(ARRAY( \n"
                                    ...
                                    "WHERE oid in (SELECT unnest(nodeoids) FROM pg_catalog.pgxc_class WHERE pcrelid = '%s' union "
                                    "SELECT unnest(coldnodeoids) FROM pg_catalog.pgxc_class WHERE pcrelid = '%s') \n"
                                "), ', ') END as loc_nodes \n"
            ...
            if (!result)
                ...
                tuples = PQntuples(result);
#ifdef __COLD_HOT__
            idistype   = PQfnumber(result, "distype");
            idiscol    = PQfnumber(result, "discolumn");
            idiseccol  = PQfnumber(result, "disseccolumn");
            ilocnodes  = PQfnumber(result, "loc_nodes");
            icoldnodes = PQfnumber(result, "cold_nodes");
#endif
            if (tuples > 0)
            {
                ...
                strvalue = PQgetvalue(result, 0, idiseccol);
                if (strvalue && strvalue[0] != '\0')
                {
                    printfPQExpBuffer(&buf, "%s %s(%s,%s)", dist_by,
                                    PQgetvalue(result, 0, idistype),
                                    PQgetvalue(result, 0, idiscol), 
                                    PQgetvalue(result, 0, idiseccol));
                }
                else
                {
                    char *distype = PQgetvalue(result, 0, idistype);

                    if (strcmp(distype, "REPLICATION") == 0 ||
                        strcmp(distype, "ROUND ROBIN") == 0)
                    {
                    ...
                    {
                        printfPQExpBuffer(&buf, "%s: %s(%s)", dist_by,
                                        PQgetvalue(result, 0, idistype),
                                        PQgetvalue(result, 0, idiscol));
                    }
                }
                ...
                strvalue = PQgetvalue(result, 0, icoldnodes);
                if (strvalue && strvalue[0] != '\0')
                {
                    /* Print location nodes info */
                    printfPQExpBuffer(&buf, "    Hotnodes:%s Coldnodes:%s",
                                        PQgetvalue(result, 0, ilocnodes),
                                        PQgetvalue(result, 0, icoldnodes));
                }
                else
                {
                    /* Print location nodes info */
                ...
                /* process key values */
                printfPQExpBuffer(&buf,
                        "SELECT  k.keyvalue as value, "
                        "(select array_to_string(ARRAY( SELECT node_name FROM pg_catalog.pgxc_node WHERE oid in (SELECT unnest(group_members) FROM pg_catalog.pgxc_group WHERE oid = k.nodegroup) ), ', ')) as hotnodes, "
                        "(select array_to_string(ARRAY( SELECT node_name FROM pg_catalog.pgxc_node WHERE oid in (SELECT unnest(group_members) FROM pg_catalog.pgxc_group WHERE oid = k.coldnodegroup) ), ', '))  as coldnodes "
                        "FROM pg_catalog.pgxc_key_value k where reloid = '%s';", oid);
                result = PSQLexec(buf.data);
                ...
                if (tuples > 0)
                {
                    ivalue      = PQfnumber(result, "value");
                    ihotnodes   = PQfnumber(result, "hotnodes");
                    icoldnodes  = PQfnumber(result, "coldnodes");
                    for (ituple = 0; ituple < tuples; ituple++)
                    {
                        strvalue = PQgetvalue(result, ituple, icoldnodes);
                        if (strvalue && strvalue[0] != '\0')
                        {
                            printfPQExpBuffer(&buf, "    KeyValue: %32s Hotnodes:%s Coldnodes:%s", 
                                            PQgetvalue(result, ituple, ivalue),
                                            PQgetvalue(result, ituple, ihotnodes),
                                            PQgetvalue(result, ituple, icoldnodes));
                        }
                        else
                        {
                            printfPQExpBuffer(&buf, "    KeyValue: %32s Nodes:%s", 
                                            PQgetvalue(result, ituple, ivalue),
                                            PQgetvalue(result, ituple, ihotnodes));                                
                        }    
                        printTableAddFooter(&cont, buf.data);
        ...
        if (verbose && pset.sversion >= 90500 && tableinfo.relkind == 'r' && tableinfo.relpartkind == 'p')
        {
            printfPQExpBuffer(&buf,
						"SELECT 'RANGE(' || a.attname || ')', partitions_number(c.oid),"
                            ...
                            "CASE WHEN p.partinterval_type=5  THEN p.partinterval_int || ' MONTH' "
                                ...
                                "ELSE p.partinterval_int || '' END as step\n"                                
                        "FROM pg_class c, pg_attribute a, pg_partition_interval p\n"
                        "WHERE c.oid=a.attrelid AND c.oid=p.partrelid "
                            "AND p.partpartkey=a.attnum AND c.oid='%s'",
                        oid);
            result = PSQLexec(buf.data);
            ...
            if (tuples > 0)
            {
                ...
                printfPQExpBuffer(&buf, "%s: %s", part_by,
                                    PQgetvalue(result, 0, 0));
                printTableAddFooter(&cont, buf.data);
    
                /* Print location nodes info */
                printfPQExpBuffer(&buf, "     %s: %s", nparts,
                                    PQgetvalue(result, 0, 1));
                printTableAddFooter(&cont, buf.data);
    
                /* Print start value */
                partdatatype = PQgetvalue(result, 0, 2);
                if(strcmp(partdatatype,"1114") == 0 || strcmp(partdatatype,"1184") == 0)
                {
                    printfPQExpBuffer(&buf, "     %s: %s", start_with,
                                    PQgetvalue(result, 0, 3));
                }
                else if(strcmp(partdatatype,"20") == 0 || strcmp(partdatatype,"21") == 0 || strcmp(partdatatype,"23") == 0)
                {
                    printfPQExpBuffer(&buf, "     %s: %s", start_with,
                ...
                /* Print interval */
                printfPQExpBuffer(&buf, "     %s: %s", interv,
...
{// #lizard forgives
    ...
    if (verbose)
    {
        ...
        if(pset.sversion >= 100000)
        {
            appendPQExpBuffer(&buf,
                              ...
                              gettext_noop("Size"));
            appendPQExpBuffer(&buf,
                              ",\n  pg_catalog.pg_size_pretty(pg_catalog.pg_allocated_table_size(c.oid)) as \"%s\"",
                              gettext_noop("Allocated Size"));
        }
        else if (pset.sversion >= 90000)
        {
            appendPQExpBuffer(&buf,
    ...
    if (!pattern)
    {
        ...
        {
            appendPQExpBufferStr(&buf, "  AND c.relpartkind != 'c'\n");
        }
    }
#endif
