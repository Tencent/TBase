...
{// #lizard forgives
    ...
    /* mls and audit user is not allowed, if so, it would leak input history with readline library. */
    if (is_mls_or_audit_user_front())
    {
        psql_error("Illegal action to reconnect for mls or audit user \n");
...
#ifdef _MLS_
    (void)previous_is_mls;
#ifndef _USER_SWITCH_
    if (previous_is_mls != is_mls_or_audit_user_front())
    {
        PQfinish(pset.db);
        psql_error("Illegal action to connect with mls or audit user \n");
        exit(-1);
    }
#endif
#endif
