...
#ifdef _SHARDING_
static SimpleOidList  dump_shards = {NULL, NULL};
static char * shardstring = NULL;
static bool with_dropped_column = false;
#endif
...
{// #lizard forgives
    ...
    static struct option long_options[] = {
        ...
        {"jobs", 1, NULL, 'j'},
#ifdef _SHARDING_
        {"shards", required_argument, NULL, 'r'},
#endif
    ...
    while ((c = getopt_long(argc, argv, "abBcCd:E:f:F:h:j:n:N:oOp:r:RsS:t:T:uU:vwWxZ:",
                            long_options, &optindex)) != -1)
    {
        switch (c)
        {
            ...
            case 'r':
                if(!optarg)
                {
                    fprintf(stderr, _("value of shard is invalid"));
                    exit_nicely(1);
                }
                simple_shard_list_append(&dump_shards, optarg);
                break;
#endif
            case 'p':            /* server port */
...
#ifdef _SHARDING_
        if(dump_shards.head != NULL)
        {
            SimpleOidListCell *cell = dump_shards.head;
            PQExpBuffer shardbuf = createPQExpBuffer();
            bool isfirst = true;
            appendPQExpBuffer(shardbuf, " (");
            while(cell)
            {                
                if(isfirst)
                {
                ...
                appendPQExpBuffer(shardbuf, "%d", cell->val);
                cell = cell->next;
            }
    
            appendPQExpBuffer(shardbuf, ")");
    
            shardstring = shardbuf->data;
        }
#endif
...
help(const char *progname)
{
    ...
    printf(_("  -O, --no-owner               skip restoration of object ownership in\n"
             "                               plain-text format\n"));
    printf(_("  -r, --shards                 dump only given shard\n"));
...
{// #lizard forgives
    ...
    if (oids && hasoids)
    {
        ...
        if(shardstring)
        {
            if(tdinfo->tdtable->pgxclocatortype == 'S')
                appendPQExpBuffer(q, "COPY %s %s WITH OIDS SHARDING %s TO stdout;",
...
#ifdef _SHARDING_
        if(shardstring)
            fprintf(stderr, _("WARNING: shards is invalid when dump from query.\n"));
#endif
    ...
    else
    {
        ...
        {
            if(tdinfo->tdtable->pgxclocatortype == 'S')
                appendPQExpBuffer(q, "COPY %s %s SHARDING %s TO stdout;",
                              fmtQualifiedId(fout->remoteVersion,
                                             tbinfo->dobj.namespace->dobj.name,
                                             classname),
                              column_list,
...
{// #lizard forgives
    ...
    if(shardstring)
    {
        appendPQExpBuffer(q, "DECLARE _pg_dump_cursor CURSOR FOR "
                              "SELECT * FROM ONLY %s WHERE shardid IN %s",
                              fmtQualifiedId(fout->remoteVersion,
                                             tbinfo->dobj.namespace->dobj.name,
                                             classname),
                                shardstring);
            ...
            if (RELKIND_RELATION == tblinfo[i].relkind)
            {
                ...
                appendPQExpBuffer(query,
                                "select "     
                                    ...
                                    "CASE "
                                        "WHEN k.nodegroup = 0 THEN 'dummygroup' "
                                        "ELSE (SELECT g1.group_name from pgxc_group g1, pgxc_key_value k1 where k1.nodegroup = g1.oid and k1.reloid = %u and k1.keyvalue = k.keyvalue) "
                                    "END as hotgroup, "       
                                    "CASE "
                                        "WHEN k.coldnodegroup = 0 THEN NULL "
                                        "ELSE (SELECT g1.group_name from pgxc_group g1, pgxc_key_value k1 where k1.coldnodegroup = g1.oid and k1.reloid = %u and k1.keyvalue = k.keyvalue) "
                                    "END as coldgroup "    
                                "FROM pgxc_key_value k ,pg_class c ,pg_namespace n "
                                "WHERE c.oid = k.reloid and c.relnamespace = n.oid and c.oid = %u;",
                ...
                if (ntups)
                {
                    i_schema    = PQfnumber(res, "schema");
                    i_table     = PQfnumber(res, "table");
                    i_keyvalue  = PQfnumber(res, "keyvalue");
                    i_hotgroup  = PQfnumber(res, "hotgroup");
                    i_coldgroup = PQfnumber(res, "coldgroup");
                    tblinfo[i].nKeys    = ntups;
                    tblinfo[i].keyvalue = (KeyValueInfo *) pg_malloc0(ntups * sizeof(KeyValueInfo));
                    for (tupid = 0; tupid < ntups; tupid++)
                    {
                        tblinfo[i].keyvalue[tupid].schema   = pg_strdup(PQgetvalue(res, tupid, i_schema));
                        tblinfo[i].keyvalue[tupid].table    = pg_strdup(PQgetvalue(res, tupid, i_table));
                        tblinfo[i].keyvalue[tupid].keyvalue = pg_strdup(PQgetvalue(res, tupid, i_keyvalue));
                        tblinfo[i].keyvalue[tupid].hotdisgroup = pg_strdup(PQgetvalue(res, tupid, i_hotgroup));
                        if (PQgetvalue(res, tupid, i_coldgroup))
                        {
                            tblinfo[i].keyvalue[tupid].colddisgroup = pg_strdup(PQgetvalue(res, tupid, i_coldgroup));
                        }
                    }
                }
                PQclear(res);
...
{// #lizard forgives
    ...
    if (fout->remoteVersion >= 90600)
    {
        ...
        appendPQExpBuffer(query,
                         ...
                         /* start value */
                         "(SELECT CASE WHEN p.partdatatype=1114    THEN 'timestamp without time zone ''' || p.partstartvalue_ts::date::text || '''' "
                                      "WHEN p.partdatatype=1184  THEN 'timestamp with time zone ''' || p.partstartvalue_ts::date::text || '''' "
                                      "WHEN p.partdatatype=20  THEN p.partstartvalue_int::text "
                                      "WHEN p.partdatatype=21  THEN p.partstartvalue_int::text "
                                      "WHEN p.partdatatype=23  THEN p.partstartvalue_int::text END "
                              "FROM pg_partition_interval p WHERE p.partrelid = c.oid) AS partstartvalue,"
                         /* interval value */
                         "(SELECT CASE WHEN p.partinterval_type=5  THEN 'INTERVAL ''' || p.partinterval_int || ' MONTH''' "
                              "WHEN p.partinterval_type=4 THEN 'INTERVAL ''' || p.partinterval_int || ' DAY''' "
                              "ELSE p.partinterval_int || '' END FROM pg_partition_interval p WHERE p.partrelid = c.oid) AS partinterval,"    
    ...
    *numTables = ntups;
    ...
    i_partstartvalue = PQfnumber(res, "partstartvalue");
    i_partinterval = PQfnumber(res, "partinterval");    
    i_groupname = PQfnumber(res, "groupname");
    i_coldgroupname = PQfnumber(res, "coldgroupname");
#endif
    ...
    for (i = 0; i < ntups; i++)
    {
        ...
        if (fout->isPostgresXL)
        {
            ...
            if (PQgetisnull(res, i, i_pgxcsecattnum))
            {
                tblinfo[i].pgxcsecattnum = 0;
            }
            else
            {
                tblinfo[i].pgxcsecattnum = atoi(PQgetvalue(res, i, i_pgxcsecattnum));
            }
        }
#endif
#ifdef __TBASE__
        ...
        {
            tblinfo[i].partattnum = atoi(PQgetvalue(res, i, i_partattnum));
            tblinfo[i].nparts = atoi(PQgetvalue(res, i, i_nparts));
            tblinfo[i].partstartvalue = pg_strdup(PQgetvalue(res, i, i_partstartvalue));
            tblinfo[i].partinterval = pg_strdup(PQgetvalue(res, i, i_partinterval));
        ...
        if (PQgetisnull(res, i, i_groupname))
            tblinfo[i].groupname = NULL;
        else
            tblinfo[i].groupname = pg_strdup(PQgetvalue(res, i, i_groupname));

        if (PQgetisnull(res, i, i_coldgroupname))
            tblinfo[i].coldgroupname = NULL;
        else
            tblinfo[i].coldgroupname = pg_strdup(PQgetvalue(res, i, i_coldgroupname));
#endif
    ...
    else
    {
        ...
        if (tbinfo->relkind != RELKIND_MATVIEW)
        {
            ...
            if (tbinfo->relkind == RELKIND_RELATION)
            {
                if (tbinfo->parttype == 'p')
                {
                    ...
                    fmtId(tbinfo->attnames[tbinfo->partattnum - 1]),
        ...
        if (tbinfo->nKeys)
        {
            ...
            for (i = 0; i < tbinfo->nKeys; i++)
            {
                ...
                {                    
                    appendPQExpBuffer(q, "CREATE KEY VALUES '%s' FOR TABLE %s.%s TO GROUP %s %s;\n", 
                ...
                {                
                    appendPQExpBuffer(q, "CREATE KEY VALUES '%s' FOR TABLE %s.%s TO GROUP %s;\n", 
                                            tbinfo->keyvalue[i].keyvalue,
                                            tbinfo->keyvalue[i].schema,
                                            tbinfo->keyvalue[i].table,
                                            tbinfo->keyvalue[i].hotdisgroup);
...
#ifdef _SHARDING_
void
simple_shard_list_append(SimpleOidList *list, const char *val)
{
    SimpleOidListCell *cell;

    /* this calculation correctly accounts for the null trailing byte */
    cell = (SimpleOidListCell *)
        pg_malloc(sizeof(SimpleOidListCell));

    cell->next = NULL;
    cell->val = (Oid)atooid(val);

    if(!ShardIDIsValid(cell->val))
    {
        fprintf(stderr, _("shardid %d is invalid\n"), cell->val);
        exit_nicely(1);
    }
