...
{// #lizard forgives
    ...
    else
    {
        ...
        snprintf(buf, sizeof(buf),
                 "\"%s%sinitdb\" --nodename %s --nodetype %s -D \"%s/%s\"  --noclean%s%s "
                 "--master_gtm_nodename %s --master_gtm_ip %s --master_gtm_port %d > \"%s/log/initdb.log\" 2>&1",
                 bindir ? bindir : "",
                 bindir ? "/" : "",
                 (char *)get_node_name(node), 
                 (PGXC_COORD_1 == node || PGXC_COORD_2 == node) ? "coordinator": "datanode",
                 temp_instance, data_folder, 
                 debug ? " --debug" : "",
                 nolocale ? " --no-locale" : "",
                 "a_one",
...
set_node_config_file(PGXCNodeTypeNum node)
{
     ...
     */
    fputs("max_prepared_transactions = 2048\n", pg_conf);
    ...
    fputs("enable_pooler_debug_print = on\n", pg_conf);
...
setup_connection_information(void)
{
    ...
    psql_command_node("postgres", PGXC_COORD_1, "create DEFAULT node group default_group with (%s, %s);",
                      (char *)get_node_name(PGXC_DATANODE_1),
                      (char *)get_node_name(PGXC_DATANODE_2));
    psql_command_node("postgres", PGXC_COORD_1, "create sharding group to group default_group;");
    psql_command_node("postgres", PGXC_COORD_1, "clean sharding;");
#endif
}
...
check_node_running(PGXCNodeTypeNum node)
{
    ...
    pwd = getpwuid(getuid());
    snprintf(buf, sizeof(buf),
             "\"%s%spsql\" -p %d -X postgres <%s 2>%s.%s",
             bindir ? bindir : "",
             bindir ? "/" : "",
             get_port_number(node), DEVNULL, "/tmp/makecheck.log", pwd->pw_name);
    return system(buf) == 0;
}
...
{// #lizard forgives
    ...
    if (nolocale)
    {
        ...
        putenv("LC_TIME=en_US.utf8");
        putenv("LANG=en_US.utf8");
        putenv("LC_MESSAGES=en_US.utf8");
        doputenv("PGCLIENTENCODING", "utf8");
        nolocale = false;
#endif
...
help(void)
{
    ...
    printf(_("  --txn-test                run cases for transaction feature\n"));
    printf(_("\n"));
