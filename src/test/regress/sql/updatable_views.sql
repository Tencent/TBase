...
CREATE VIEW rw_view1 AS SELECT * FROM base_tbl WHERE a>0 order by 1;
...
CREATE VIEW rw_view1 AS SELECT b AS bb, a AS aa FROM base_tbl WHERE a >0 order by 1;
...
CREATE VIEW rw_view1 AS SELECT * FROM base_tbl WHERE a>0 order by 1 OFFSET 0; -- not updatable without rules/triggers
...
CREATE VIEW rw_view1 AS SELECT b AS bb, a AS aa FROM base_tbl order by 1;
...
CREATE VIEW rw_view1 AS SELECT b AS bb, c AS cc, a AS aa FROM base_tbl order by 1;
...
CREATE VIEW rw_view2 AS SELECT b AS bb, c AS cc, a AS aa FROM base_tbl order by 1;
...
CREATE VIEW rw_view1 AS SELECT a AS aa, b AS bb FROM base_tbl order by 1;
...
CREATE VIEW rw_view1 AS SELECT * FROM base_tbl_parent order by 1;
