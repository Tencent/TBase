...
--
SET SESSION AUTHORIZATION regress_rls_bob;
CREATE VIEW rec1v AS SELECT * FROM rec1 order by 1;
CREATE VIEW rec2v AS SELECT * FROM rec2 order by 1;
SET SESSION AUTHORIZATION regress_rls_alice;
...
SET SESSION AUTHORIZATION regress_rls_bob;
CREATE VIEW v2 AS SELECT * FROM s2 WHERE y like '%af%' order by 1;
...
SET SESSION AUTHORIZATION regress_rls_bob;
SELECT * FROM s1 WHERE f_leak(b) order by 1;	-- OK
...
SELECT (SELECT x FROM s1 LIMIT 1) xx, * FROM s2 WHERE y like '%28%' order by 1;
...
PREPARE p1(int) AS SELECT * FROM t1 WHERE a <= $1 order by 1;
EXECUTE p1(2);
...
PREPARE p2(int) AS SELECT * FROM t1 WHERE a = $1 order by 1;
EXECUTE p2(2);
...
SELECT * FROM bv1 WHERE f_leak(b) order by 1;
...
SET SESSION AUTHORIZATION regress_rls_alice;
CREATE VIEW rls_view AS SELECT * FROM z1 WHERE f_leak(b) order by 1;
...
SET SESSION AUTHORIZATION regress_rls_bob;
SELECT * FROM y2 WHERE f_leak(b) order by 1;
EXPLAIN (COSTS OFF) SELECT * FROM y2 WHERE f_leak(b);

--
-- Qual push-down of leaky functions, when not referring to table
--
SELECT * FROM y2 WHERE f_leak('abc') order by 1;
...
SELECT * FROM y2 JOIN test_qual_pushdown ON (b = abc) WHERE f_leak(abc) order by 1;
EXPLAIN (COSTS OFF) SELECT * FROM y2 JOIN test_qual_pushdown ON (b = abc) WHERE f_leak(abc);

SELECT * FROM y2 JOIN test_qual_pushdown ON (b = abc) WHERE f_leak(b) order by 1;
