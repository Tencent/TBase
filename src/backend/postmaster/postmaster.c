...
bool        g_enable_gtm_proxy = false;
char       *g_BouncerConf = "pgbouncer.ini";
bool        enable_null_string = false;
bool        g_concurrently_index = false;
bool        exit_on_stop = false;
bool        g_set_global_snapshot = false;
#endif

#ifdef __COLD_HOT__
int32    g_create_key_value_mode = 0;
int      g_ColdDataThreashold   = 3650;  /* cold data threashold, 10 years, meaning disabled */
struct pg_tm   g_ManualHotDataTime; 
int32    g_ManualHotDataGapWithMonths    = 0;
int32    g_ManualHotDataGapWithDays    = 0;
char     *g_ManualHotDate      = NULL;
char     *g_TempColdDate        = NULL;
struct pg_tm      g_TempColdDataTime; 
char     *g_TempKeyValue        = NULL;
char     *g_TempHotDate        = NULL;
struct pg_tm   g_TempHotDataTime;        /* set as temporary cold data, when check data consistency in cold datanode*/

char    *g_ColdHotPartitionMode = NULL;
int      g_ColdHotPartitionType = 0;
#endif
...
static void InitPostmasterLatch(void);
#endif
/*
...
{// #lizard forgives
    ...
    for (;;)
    {
        ...
        if (AuditLoggerPID == 0)
        {
            ...
            {
                AuditLoggerPID = AuditLogger_Start();
                if (enable_auditlogger_warning)
                {
                    ereport(LOG,
                        (errmsg("start audit logger in ServerLoop, "
                                "pmState = %d, AuditLoggerPID = %d", pmState, AuditLoggerPID)));
                }
            }
        }
#endif
    ...
    for (i = MaxLivePostmasterChildren() - 1; i >= 0; i--)
    {
        ...
        if (bp->pid == backendPID)
        {
            if (bp->cancel_key == cancelAuthCode)
            {
                /* Found a match; signal that backend to cancel current op */
                ereport(DEBUG2,
                        (errmsg_internal("processing end query request: sending SIGUSR2 to process %d",
                                         backendPID)));
                signal_child(bp->pid, SIGUSR2);
            }
            else
                /* Right PID, wrong key: no way, Jose */
                ereport(LOG,
                        (errmsg("wrong key in end query request for process %d",
                                backendPID)));
            return;
        }
    }
        ...
        /* No matching backend */
    ereport(LOG,
            (errmsg("PID %d in end query request did not match any process",
                    backendPID)));
}
#endif
    ...
    switch (postgres_signal_arg)
    {
        case SIGTERM:
                ...
                pmState == PM_HOT_STANDBY || pmState == PM_STARTUP)
            {
                ...
                if (AuditLoggerPID != 0)
                {
                    ...
                    ereport(LOG,
                            (errmsg("Smart Shutdown: SIGTERM : send %d to audit logger %d", audit_sig, AuditLoggerPID)));
                    signal_child(AuditLoggerPID, audit_sig);
                }
#endif
                /*
        ...
        case SIGINT:
            ...
            if (AuditLoggerPID != 0)
            {
                ...
                ereport(LOG,
                    (errmsg("Fast Shutdown: SIGINT: send %d to audit logger %d", audit_sig, AuditLoggerPID)));
                signal_child(AuditLoggerPID, audit_sig);
            }
#endif
    ...
    while ((pid = waitpid(-1, &exitstatus, WNOHANG)) > 0)
    {
        ...
        if (pid == StartupPID)
        {
            ...
            if (AuditLoggerPID == 0)
            {
                ...
                {
                    ereport(LOG,
                        (errmsg("start audit logger in reaper [pid == StartupPID], "
                                "pmState = %d, AuditLoggerPID = %d", pmState, AuditLoggerPID)));
                }
            }
#endif
        ...
        {    
            ...
            if (pmState == PM_RUN || pmState == PM_HOT_STANDBY)
            {
                ...
                {
                    ereport(LOG,
                        (errmsg("start audit logger in reaper [pid == AuditLoggerPID], "
                                "pmState = %d, AuditLoggerPID = %d, OldAuditLoggerPID = %d", pmState, AuditLoggerPID, pid)));
                }
            }

            if (!EXIT_STATUS_0(exitstatus))
                LogChildExit(LOG, _("audit logger process"),
                             pid, exitstatus);
            continue;
        }
#endif
        ...
        if (pid == BouncerPid && IS_PGXC_COORDINATOR && g_enable_bouncer)
        {
            ...
            if (!EXIT_STATUS_0(exitstatus))
                LogChildExit(LOG, _("bouncer process"),
                             pid, exitstatus);
            continue;
        }
    ...
    slist_foreach_modify(iter, &BackgroundWorkerList)
    {
        ...
        if (!EXIT_STATUS_0(exitstatus))
        {
            ...
            if ((rw->rw_worker.bgw_flags & BGWORKER_CLASS_PARALLEL) != 0
                && notify_pid != 0)
                SendProcSignal(notify_pid, PROCSIG_PARALLEL_EXIT, InvalidBackendId);
        }
#endif
...
HandleChildCrash(int pid, int exitstatus, const char *procname)
{
    ...
    if (pid == AuditLoggerPID)
        AuditLoggerPID = 0;
    else if (AuditLoggerPID != 0 && take_action)
    {
        ereport(DEBUG2,
                (errmsg_internal("sending %s to process %d",
                                 (SendStop ? "SIGSTOP" : "SIGQUIT"),
                                 (int) AuditLoggerPID)));
        signal_child(AuditLoggerPID, (SendStop ? SIGSTOP : SIGQUIT));
    }
#endif
...
TerminateChildren(int signal)
{
    ...
    {
        ereport(LOG,
            (errmsg("TerminateChildren: send %d to audit logger %d", signal, AuditLoggerPID)));
        signal_child(AuditLoggerPID, signal);
    }
#endif
}
...
SubPostmasterMain(int argc, char *argv[])
{
    ...
    if (strcmp(argv[1], "--forkalog") == 0)
    {
        ...
        AuditLoggerMain(argc, argv);    /* does not return */
    }
#endif
...
sigusr1_handler(SIGNAL_ARGS)
{
    ...
    if (CheckPostmasterSignal(PMSIGNAL_ROTATE_AUDIT_LOGFILE) &&
        AuditLoggerPID != 0)
    {
        /* Tell audit logger to rotate logfile */
        signal_child(AuditLoggerPID, SIGUSR1);        
    }

    if (CheckPostmasterSignal(PMSIGNAL_WAKEN_AUDIT_LOGGER) &&
        AuditLoggerPID != 0)
    {
        /* Tell audit logger to read audit log */
        signal_child(AuditLoggerPID, SIGUSR2);
    }
#endif
    ...
    if (CheckPostmasterSignal(PMSIGNAL_WAKEN_AUDIT_FGA_TRIGGER) &&
        AuditFgaBgworkerPID != 0)
    {
        /* Tell audit_fga bgworker to  trigger function */
        signal_child(AuditFgaBgworkerPID, SIGUSR1);
    }
#endif
        ...
        default:
            ...
            /*
              * record audit fga bgwriter
                    */
            if ((rw->rw_worker.bgw_flags & BGWORKER_CLASS_AUDIT_FGA) != 0)
            {
                AuditFgaBgworkerPID = worker_pid;
            }
#endif
            return true;
    }
...
static int32 StartBouncer(void)
{
    ...
    switch ((BouncerPid = fork_process()))
    {
        case -1:
            {
                ereport(LOG,
                        (errmsg("could not fork pgbouncer : %m")));
                return 0;
            }
        ...
        case 0:
            {
                /* in bouncer child ... */            
                snprintf(conf, 1024, "%s", my_exec_path);
                p  = strstr(conf, "postgres");
                if(p)
                    *p = '\0';
                snprintf(bouncer, 1024, "%s/pgbouncer", conf);
                snprintf(conf, 1024, "%s/%s", DataDir, g_BouncerConf);

                snprintf(postmaster, 64, "%d", PostmasterPid);

                /* do the work , start bouncer process */
                ret = execl(bouncer, "pgbouncer", "-P", postmaster, "-f", "postgres",conf, (char *) NULL);
                return ret;
            }
        default:
            {
                /* parent */
                return (int) BouncerPid;
            }
    }
...
static int32 StartGTMProxy(void)
{     
#define   GTM_PROXY_RESTART_GAP 60 /* We try to start the gtm proxy only GTM_PROXY_RESTART_GAP seconds later. */
    int    ret = 0;
    char   conf[1024];
    char   proxy[1024];
    ...
    else
    {
        ...
        if (current_time - last_start_time < GTM_PROXY_RESTART_GAP)
        {
            return 0;
        }
    ...
    switch ((GTMProxyPID = fork_process()))
    {
        case -1:
            {
                ereport(LOG,
                        (errmsg("could not fork gtm_proxy: %m")));
                return 0;
            }
        ...
        case 0:
            {
                ...
                p  = strstr(conf, "postgres");
                *p = '\0';
                snprintf(proxy, 1024, "%s/gtm_proxy", conf);

                /* do the work , start gtm_proxy process */
                ret = execl(proxy, "gtm_proxy", "-D", DataDir, (char *) NULL);
                return ret;
            }
        default:
            {
                /* parent */
                return (int) GTMProxyPID;
            }
    }
...
static void
PostmasterLogTimeoutHandler(void)
{
    /* do nothing */
}

void PostmasterEnableLogTimeout(void)
{    
    if (MyProcPid == PostmasterPid)
    {
        enable_timeout_after(g_postmaster_timeout_id, 2000); /* write timeout 2 seconds */
    }
}

void PostmasterDisableTimeout(void)
{    
    if (MyProcPid == PostmasterPid)
    {
