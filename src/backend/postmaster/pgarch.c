...
int archive_status_control = ARCHSTATUS_CONTINUE;

/* How often to force a poll of the archive status directory; in seconds. */
int archive_autowake_interval = PGARCH_AUTOWAKE_INTERVAL;

static bool 
pgarch_Break()
{
    return (bool)(archive_status_control == ARCHSTATUS_BREAK);
}
...
{// #lizard forgives
    ...
    do
    {
        ...
        {
            ereport(WARNING,
                    (errmsg("archive_mode enabled, yet archive_status_control is set to break .")));
            pg_usleep(1000000L * SLEEPSECONDS);
            continue;
        }
#endif
...
{// #lizard forgives
    ...
    while (loop < 2)
    {
        ...
        {
            /* the second one we backup the gts file */
            snprintf(pathname, MAXPGPATH, XLOGDIR "/archive_status/%s.gts", xlog);
            snprintf(xlog_gts, MAXFNAMELEN, "%s.gts", xlog);            
        }
#endif
