        ...
        case ACL_OBJECT_SEQUENCE:
            foreach(cell, objnames)
            {
                ...
                if (objtype == ACL_OBJECT_RELATION)
                {
                    ...
                    if (rel->rd_rel->relkind == RELKIND_RELATION && RELATION_IS_INTERVAL(rel))
                    {
                        List *childoids = RelationGetAllPartitions(rel);

                        objects = list_concat(objects, childoids);
                    }
...
#ifdef _MLS_
List * mls_get_relid_list_in_schema(Oid schemaoid)
{
    List       *objects = NIL;
    List       *objs = NIL;

    objs = getRelationsInNamespace(schemaoid, RELKIND_RELATION);
    objects = list_concat(objects, objs);
    objs = getRelationsInNamespace(schemaoid, RELKIND_PARTITIONED_TABLE);
    objects = list_concat(objects, objs);
