...
{    
    UserAuthTag       tag;
    TimestampTz       last_login_failed_timestamp;
    int               fail_cnt;
    int               lock_time; /* unit: second, account will be lock for lock_time seconds. */            
} UserAuthEntry;

static HTAB               *g_UserAuthHashTab     = NULL;
int                           pgua_max;            /* max # user authentication info to record */
int                           account_lock_time     = 600; /* 10 mins */
int                           account_lock_threshold = 10; /* if failed_cnt >= 10, lock account for account_lock_time seconds */
bool                        enable_lock_account     = true;
bool                       lock_account_print    = true;
#endif
...
{// #lizard forgives
    ...
    switch (port->hba->auth_method)
    {
        ...
        case uaSCRAM:
            ...
            {
                errstr = gettext_noop("account locked for user \"%s\". try again after \"%d\" seconds");
    ...
    ereport(FATAL,
            (errcode(errcode_return),
             status == STATUS_LOCK ? errmsg(errstr, port->user_name, port->lock_time) : errmsg(errstr, port->user_name),
...
{// #lizard forgives
    ...
    inner_conn = mls_check_inner_conn(port->cmdline_options);
#endif
...
#define HOSTNAME_LOOKUP_DETAIL(port) \
        ...
        case uaSCRAM:
            ...
            {
                status = pgua_check(port);
                if (lock_account_print)
                {
                    elog(LOG, "[ClientAuthentication]MD5 pgua_check return status:%d", status);
                }
                if (status != STATUS_LOCK)
                {
        ...
        case uaPassword:
            ...
            if (lock_account_print)
            {
                elog(LOG, "[ClientAuthentication]Password pgua_check return status:%d", status);
            }
            if (status != STATUS_LOCK)
            {
...
#ifdef  _MLS_
    if(pg_strncasecmp(port->user_name,
                      MLS_USER_PREFIX,
...
CheckPasswordAuth(Port *port, char **logdetail)
{
    ...
    pgua_store(port->user_name, result);
...
Size UserAuthShmemSize(void)
{
    ...
    size = mul_size(pgua_max * 2 , MAXALIGN64(sizeof(UserAuthEntry)));
    return size;
}

void UserAuthShmemInit(void)
{
    HASHCTL        info;

    /* init hash table */
    info.keysize   = sizeof(UserAuthTag);
    info.entrysize = sizeof(UserAuthEntry);
    info.hash = tag_hash;

    g_UserAuthHashTab = ShmemInitHash("User authentication info ",
                                      pgua_max, 
                                      pgua_max,
                                      &info,
                                      HASH_ELEM | HASH_FUNCTION);    

    if (!g_UserAuthHashTab)
    {
        elog(FATAL, "invalid shmem status when creating user auth hash ");
    }
}

/* pg user authentication info store */
static void pgua_store(const char* username, int result)
{
    UserAuthTag     tag;
    bool            found;
    UserAuthEntry*    ent = NULL;

    if (!username)
        return;

    if (!enable_lock_account)
        return;
    
    /* Get the target role's OID.  Note we do not error out for bad role. */
    tag.userid = get_role_oid(username, true);
    
    if (InvalidOid == tag.userid)
    {
        elog(LOG, "Failed to find oid of user:%s", username);
        return;
    }
    
    LWLockAcquire(UserAuthLock, LW_EXCLUSIVE);

    ent = (UserAuthEntry*)hash_search(g_UserAuthHashTab, (void *) &tag, HASH_ENTER, &found);    
    
    {
        if (lock_account_print)
        {
            elog(LOG, "[pgua_store] Before udpate entry. found:%d, result=%d", found, result);
            PrintUserAuthEntryInfo(ent);
        }
        
        /* if auth ok, clear the info in hash table */
        if (STATUS_OK == result)
        {
            ent->fail_cnt = 0;
            ent->last_login_failed_timestamp = 0;
        }
        else
        {
            ent->fail_cnt++;
            ent->last_login_failed_timestamp = GetCurrentTimestamp();
            ent->lock_time = account_lock_time;
        }

        if (lock_account_print)
        {
            elog(LOG, "[pgua_store] After udpate entry. found:%d, result=%d", found, result);
            PrintUserAuthEntryInfo(ent);
        }
    }
    
    LWLockRelease(UserAuthLock);
...
{// #lizard forgives
    ...
    if (!username)
    {
        if (lock_account_print)
        {
            elog(LOG, "[pgua_check] username not provided");
        }
        return STATUS_OK;
    }
    ...
    if (!enable_lock_account)
    {
        if (lock_account_print)
        {
            elog(LOG, "[pgua_check] lock_account disabled");
        }
        return STATUS_OK;
    }
    ...
    {
        elog(LOG, "Failed to find oid of user:%s", username);
        /* Here we return ok, let other module report error to client */
        return STATUS_OK;
    }

    LWLockAcquire(UserAuthLock, LW_SHARED);

    ent = (UserAuthEntry*)hash_search(g_UserAuthHashTab, (void *) &tag, HASH_FIND, &found);    
    
    if (lock_account_print)
    {
        elog(LOG, "[pgua_check] search entry for user:%s userid:%d. found:%d", username, tag.userid, found);
        if (found)
            PrintUserAuthEntryInfo(ent);
    }
    ...
    {
        /* reject user login */
        if (ent->fail_cnt >= account_lock_threshold)
        {
            TimestampDifference(ent->last_login_failed_timestamp, GetCurrentTimestamp(), &secs, &microsecs);
            if (secs == 0 && microsecs == 0)
            {
                /* should not happen */
                elog(ERROR, "timediff compute error");
            }

            /* In the time range of lock, tell client still need to lock for # seconds */
            if (secs < ent->lock_time)
            {
                port->lock_time = ent->lock_time - secs;
                retval = STATUS_LOCK;
            }
        }
    }
...
static void PrintUserAuthEntryInfo(UserAuthEntry* ent)
{
    char        formatted_time[FORMATTED_TS_LEN] = {0};
    pg_time_t     time;
    ...
    if (lock_account_print)
    {
        time = timestamptz_to_time_t(ent->last_login_failed_timestamp);
        pg_strftime(formatted_time, FORMATTED_TS_LEN,
                    "%Y-%m-%d %H:%M:%S %Z",
                    pg_localtime(&time, log_timezone));
    
        elog(LOG, "[PrintUserAuthEntryInfo]userid:%d, last_login_failed_timestamp:%s, fail_cnt:%d, lock_time:%d", 
                    ent->tag.userid, formatted_time, ent->fail_cnt, ent->lock_time);
    }
}
