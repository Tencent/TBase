...
{// #lizard forgives
    ...
    else
    {
        ...
        em2 = add_eq_member(ec, item2, item2_relids, item2_nullable_relids,
                            false, item2_type);
#if 0
        if (IS_PGXC_COORDINATOR && root->parse && 
            (root->parse->commandType == CMD_UPDATE || root->parse->commandType == CMD_DELETE))
        {
            Node *left_arg = (Node *) get_leftop(clause);
            Node *right_arg = (Node *) get_rightop(clause);
            Var  *left_var = (Var *)get_var_from_arg(left_arg);
            Var  *right_var = (Var *)get_var_from_arg(right_arg);
            if (is_var_distribute_column(left_var, root->parse->rtable) && 
                is_var_distribute_column(right_var, root->parse->rtable))
            {
                ec->joinable_expr = true;
            }
        }
#endif
