...
{// #lizard forgives
    ...
    foreach(l, onconflict->arbiterElems)
    {
        ...
        if (root->parse->hasUnshippableTriggers)
        {
            root->parse->conflict_cols = bms_add_member(root->parse->conflict_cols, attno);
        }
#endif
    }
        ...
        case RELKIND_TOASTVALUE:
            ...
            if (IS_PGXC_COORDINATOR && rel->rd_locator_info != NULL)
            {
                ...
                if (rel->rd_rel->relkind == RELKIND_RELATION)
                {
                    ...
                    {
                        curpages = GetIntervalPartitionPages(rel, false, true);
                    }
                    else
                ...
                {
                    /* get relation which index on */
                    Oid indexRelid = IndexGetRelationId(rel);
                        
                    if (IsIntervalPartition(indexRelid))
                    {
                        curpages = GetIntervalPartitionPages(rel, true, true);
                    }
                    else
                        curpages = rel->rd_rel->relpages;
                }
...
#ifdef __TBASE__
            {
                if (rel->rd_rel->relkind == RELKIND_RELATION)
                {
                    if (RELATION_IS_INTERVAL(rel))
                    {
                        curpages = GetIntervalPartitionPages(rel, false, false);
                    }
                    else
                        curpages = rel->rd_rel->relpages;
                }
                else if (rel->rd_rel->relkind == RELKIND_INDEX)
                {
                    ...
                    if (IsIntervalPartition(indexRelid))
                    {
                        curpages = GetIntervalPartitionPages(rel, true, false);
                    }
                    else
                        curpages = rel->rd_rel->relpages;
                }
...
#ifdef _MLS_
                   || att_tup->atthasmissing
#endif
                )
                {
                    /* found a dropped col or missing , so punt */
                    tlist = NIL;
                    break;
                }
...
{// #lizard forgives
    ...
    {
        Oid relationId = IndexGetRelationId(rel);

        indexRel = heap_open(relationId, AccessShareLock);

        nparts = RelationGetNParts(indexRel);

        heap_close(indexRel, AccessShareLock);
    }
    
    if (nparts <= ParentTablePageSampleThreshold)
    {
        for (partidx = 0; partidx < nparts; partidx++)
        {
            ...
            if (isindex)
                ...
                childrel = heap_open(partoid, AccessShareLock);

            if (statistic)
                blocknum += childrel->rd_rel->relpages;
            else
                blocknum += RelationGetNumberOfBlocks(childrel);

            if (isindex)
                index_close(childrel, AccessShareLock);
            else
    ...
    {
        step = 100 / ParentTablePageSampleRate;
        scannum    = nparts * ParentTablePageSampleRate / 100;
