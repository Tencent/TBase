...
#ifdef _MLS_
extern bool g_enable_crypt_parellel_debug;
#endif
...
#ifdef _MLS_
int         g_crypt_buffer_cnt;
int         g_normal_buffer_cnt;
extern void print_page_header(PageHeader header);
#endif
...
enum
{
    SYNC_BUF_KEEP_GOING       = 0,
    SYNC_BUF_BREAK            = 1,
    SYNC_BUF_LWLOCK_CONFLICT  = 2,
    SYNC_BUF_BUTT
};

static int CheckBufferNeedSync(int buf_id, bool skip_recently_used, int *buf_ret_status);
static int SyncBufferPrePhase1(int buf_id);
static int SyncBufferPrePhase2(int buf_id);
static List* SyncBufferDoing(int buf_id, int status);
    ...
    else
    {
        ...
        else
        {
...
#ifdef _MLS_
            /* before verify, decrypt if needed */
            if (MAIN_FORKNUM == forkNum || EXTENT_FORKNUM == forkNum)
            {
                algo_id = PageGetAlgorithmId(bufBlock);
                if (TRANSP_CRYPT_ALGO_ID_IS_VALID(algo_id))
                {
                    ...
                    else
                    {
                        elog(LOG, "found one page whose algo_id:%d diffs with smgr_relcrypt_algo_id:%d, relfilenode:%d:%d:%d, forknum:%d, blknum:%d",
                                    algo_id,
                                    smgr->smgr_relcrypt.algo_id,
                                    smgr->smgr_rnode.node.dbNode, smgr->smgr_rnode.node.spcNode, smgr->smgr_rnode.node.relNode,
                                    forkNum, blockNum);
                    }
                }
            }
            ...
            if (!PageIsVerified((Page) bufBlock, blockNum))
            {
                ...
                {
                    print_page_header((PageHeader)bufBlock);
                    ereport(ERROR,
                            (errcode(ERRCODE_DATA_CORRUPTED),
                             errmsg("invalid page in block %u of relation %s, forkNum:%d",
                                    blockNum,
                                    relpath(smgr->smgr_rnode, forkNum), forkNum)));
                }
            }
        }
    }
...
#ifdef _SHARDING_
Buffer
ReadBufferForRedoExtentTable(SMgrRelation smgr, BlockNumber blockNum, ReadBufferMode mode)
{
    bool hit = false;
    return ReadBuffer_common(smgr,
...
{// #lizard forgives
...
#ifdef _MLS_
    ResourceOwnerEnlargeBuffersToSize(CurrentResourceOwner, mls_crypt_parle_get_queue_capacity());
#endif
    ...
    num_written = 0;
#ifdef _MLS_
    g_crypt_buffer_cnt  = 0;
    g_normal_buffer_cnt = 0;
#endif
    ...
    if (false == mls_encrypt_queue_is_empty())
    {
        elog(LOG, "CHECKPOINT: already done: normal buffers flushed:%d, crypted buffers flushed:%d",
            g_normal_buffer_cnt, g_crypt_buffer_cnt);
    }
    
    /* wait crypt buffers finished */
    while (false == mls_encrypt_queue_is_empty())
    {
        buf_id_list = SyncBufferWaitParellelFinsih(&wb_context);
        ...
        foreach(l, buf_id_list)
        {
            ...
            if (info->status & BUF_WRITTEN)
            {
                TRACE_POSTGRESQL_BUFFER_SYNC_WRITTEN(info->buf_id);
                BgWriterStats.m_buf_written_checkpoints++;
                num_written++;
            }
        }

        list_free(buf_id_list);
    }
#endif
...
#ifdef _MLS_
    elog(LOG, "CHECKPOINT:in one round, buffer crypted write:%d, normal buffer write:%d", 
                g_crypt_buffer_cnt, g_normal_buffer_cnt);
    mls_log_crypt_worker_detail();    
#endif    
}
...
#ifdef _SHARDING_
    /* shard has been barriered. */
    if(!IsBootstrapProcessingMode() &&       /* is not bootstrap mode  and */ 
        buf->tag.forkNum == MAIN_FORKNUM && /* this block is belong to MAIN_FORKNUM and*/
        (PageIsNew(bufBlock) ||               /* page is empy or */           
            IsShardBarriered(buf->tag.rnode, PageGetShardId(bufBlock)))
                                            /* this shard is barrier by other backend */
        )
    {
        //skip flush this block to disk.
        elog(DEBUG1, "skip to flush block(%d/%d/%d/%d) with shardid(%d) to disk.",
                buf->tag.rnode.dbNode,
                buf->tag.rnode.spcNode,
                buf->tag.rnode.relNode,
                buf->tag.blockNum,
                PageGetShardId(bufBlock));
    }
    else
    {
#endif
...
#ifdef _SHARDING_
void DropRelfileNodeShardBuffers(RelFileNode rnode, ShardID sid)
{
    int            i;

    /* if it's a local relation, skip it. */
    ...
    for (i = 0; i < NBuffers; i++)
    {
        ...
        if (RelFileNodeEquals(bufHdr->tag.rnode, rnode) &&
            ...
            PageGetShardId(BufHdrGetBlock(bufHdr)) == sid)
            InvalidateBuffer(bufHdr);    /* releases spinlock */
        else
            UnlockBufHdr(bufHdr, buf_state);
    }
...
void DropRelfileNodeExtentBuffers(RelFileNode rnode, ExtentID eid)
{
    int            i;
    ...
    for (i = 0; i < NBuffers; i++)
    {
        BufferDesc *bufHdr = GetBufferDescriptor(i);
        uint32        buf_state;
        BlockNumber blk_start = eid * PAGES_PER_EXTENTS;
        BlockNumber blk_end = (eid  + 1) * PAGES_PER_EXTENTS;
        ...
        if (RelFileNodeEquals(bufHdr->tag.rnode, rnode) &&
            bufHdr->tag.forkNum == MAIN_FORKNUM &&
            bufHdr->tag.blockNum >= blk_start &&
            bufHdr->tag.blockNum < blk_end)
            InvalidateBuffer(bufHdr);    /* releases spinlock */
        else
            UnlockBufHdr(bufHdr, buf_state);
    }
...
#ifdef _MLS_
    /*
     * in parellel crypt process, there would be multi buffers in progress, 
     * so the global variable maybe not empty any more.
     * buffers in IO status would be tracked by crypt queues.
     */
...
#ifdef _MLS_
/*
 * take care of buf in progress and crypt buffers in queue if exists.
 */
void AbortBufferIOParellel(void)
{
    List          * buf_id_list = NIL;
    BufferDesc    * buf;
    ListCell      * l;
    ...
    if (buf)
    {
        buf_id_list = NormalBufidListMake(buf->buf_id, 0);
    }
...
retry:
    ...
    foreach(l, buf_id_list)
    {
        ...
        /* 
         * Issue notice if this is not the first failure... 
         * maybe is this a mistake, cause, the InProgressBuf are twice in buf_id_list,
         * however, the buf_id is logged, so it does not matter.
         */
        if (buf_state & BM_IO_ERROR)
        {
            /* Buffer is pinned, so we can read tag without spinlock */
            ...
            ereport(WARNING,
                    (errcode(ERRCODE_IO_ERROR),
                     errmsg("CHECKPOINT: could not write block %u(buf_id:%d) of %s",
                            buf->tag.blockNum, buf->buf_id, path),
                     errdetail("Multiple failures --- write error might be permanent.")));
            pfree(path);
        }
        ...
        /* after smgrwrite, slot_id could be free */
        if (INVALID_WORKER_ID != info->worker_id && INVALID_SLOT_ID != info->slot_id)
        {
            mls_crypt_worker_free_slot(info->worker_id, info->slot_id);
        }
    }
...
/*
 * check buffer status if it has been written by others, and keep the status
 */
static int CheckBufferNeedSync(int buf_id, bool skip_recently_used, int *buf_ret_status)
{
    BufferDesc *bufHdr = GetBufferDescriptor(buf_id);
...
static int SyncBufferPrePhase2(int buf_id)
{
    ...
    reln = smgropen(buf->tag.rnode, InvalidBackendId);
    if (g_enable_crypt_parellel_debug)
    {
        elog(LOG, "CHECKPOINT: step1 : smgropen rnode:%d:%d:%d, buf_id:%d, bufstate:%x", 
            buf->tag.rnode.dbNode, buf->tag.rnode.spcNode, buf->tag.rnode.relNode, buf_id, pg_atomic_read_u32(&buf->state));
    }
    TRACE_POSTGRESQL_BUFFER_FLUSH_START(buf->tag.forkNum,
                                        buf->tag.blockNum,
                                        reln->smgr_rnode.node.spcNode,
...
{// #lizard forgives
    ...
    {
        //skip flush this block to disk.
        elog(LOG, "skip to flush block(%d/%d/%d/%d) with shardid(%d) to disk.",
                buf->tag.rnode.dbNode,
                buf->tag.rnode.spcNode,
                buf->tag.rnode.relNode,
                buf->tag.blockNum,
                PageGetShardId(bufBlock));
        /* if buf is barried by shard truncate, terminate buf right now, release BM_IO_IN_PROGRESS */
        /*
         * Mark the buffer as clean (unless BM_JUST_DIRTIED has become set) and
         * end the io_in_progress state.
         */
    ...
    else
    {
            ...
            && (MAIN_FORKNUM == buf->tag.forkNum || EXTENT_FORKNUM == buf->tag.forkNum))
        {
            ...
            if (g_enable_crypt_parellel_debug)
            {
                elog(LOG, "CHECKPOINT: step2 : crypt buffer rnode:%d:%d:%d, buf_id:%d, bufstate:%x", 
                    buf->tag.rnode.dbNode, buf->tag.rnode.spcNode, buf->tag.rnode.relNode, buf_id, pg_atomic_read_u32(&buf->state));
            }
            buf_id_list = mls_encrypt_buf_parellel(buf_id_list, reln->smgr_relcrypt.algo_id, buf_id, status);
            g_crypt_buffer_cnt ++;
            /* ---------------- parellel section ---------------- */
        }
        else
        {       
            if (g_enable_crypt_parellel_debug)
            {
                elog(LOG, "CHECKPOINT: step2 : no need to crypt buffer rnode:%d:%d:%d, buf_id:%d, bufstate:%x", 
                    buf->tag.rnode.dbNode, buf->tag.rnode.spcNode, buf->tag.rnode.relNode, buf_id, pg_atomic_read_u32(&buf->state));
            }
            buf_id_list = SyncBufidListAppend(buf_id_list, buf_id, status, INVALID_SLOT_ID, INVALID_WORKER_ID, bufToWrite);
            g_normal_buffer_cnt ++;
        }
    }   
...
static void SyncBufferPostPhase2(List * buf_id_list)
{
    ...
    foreach(l, buf_id_list)
    {
        ...
        if (g_enable_crypt_parellel_debug)
        {
            elog(LOG, "CHECKPOINT: step3 : smgrwrite rnode:%d:%d:%d, buf_id:%d, bufstate:%x", 
                buf->tag.rnode.dbNode, buf->tag.rnode.spcNode, buf->tag.rnode.relNode, info->buf_id, pg_atomic_read_u32(&buf->state));
        }
        if (track_io_timing)
            INSTR_TIME_SET_CURRENT(io_start);
        /*
...
static List* SyncBufferPostPhase1(List * buf_id_list, WritebackContext *wb_context)
{
    ...
    foreach(l, buf_id_list)
    {
        ...
        if (g_enable_crypt_parellel_debug)
        {
            elog(LOG, "CHECKPOINT: step4 : unpin smgrwrite rnode:%d:%d:%d, buf_id:%d, nitmes:%u, lastidx:%u, bufstate:%x", 
                        buf->tag.rnode.dbNode, buf->tag.rnode.spcNode, buf->tag.rnode.relNode, info->buf_id,
                        GetResourceArrayNitems(), GetResourceArrayLastidx(), pg_atomic_read_u32(&buf->state));
        }
        ScheduleBufferTagForWriteback(wb_context, &tag);

        info->status = info->status | BUF_WRITTEN;
    }
...
static List* SyncBufferPostPhase1_2(List * buf_id_list)
{
    ...
    foreach(l, buf_id_list)
    {
        ...
        if (g_enable_crypt_parellel_debug)
        {
            elog(LOG, "CHECKPOINT: step5 : unpin smgrwrite rnode:%d:%d:%d, buf_id:%d, nitmes:%u, lastidx:%u, bufstate:%x", 
                        buf->tag.rnode.dbNode, buf->tag.rnode.spcNode, buf->tag.rnode.relNode, info->buf_id,
                        GetResourceArrayNitems(), GetResourceArrayLastidx(), pg_atomic_read_u32(&buf->state));
        }
        
        info->status &= (~BUF_WRITTEN);
    }
...
/*
 * normal buf_id means buf does not need encrypt, so only bufstatus would be used for statistic collecting.
 */
static List * NormalBufidListMake(int buf_id, int bufstatus)
{
    SyncBufIdInfo  * node;
    ...
    node->buf_id        = buf_id;
    node->slot_id       = INVALID_SLOT_ID;
    node->worker_id     = INVALID_WORKER_ID;
    node->status        = bufstatus;    
    node->encrypted_buf = NULL;
    
    buf_id_list = lappend(NIL, node);

    return buf_id_list;
}

/*
 * to encrypt buffer is compute-intensive and there would be a long delay to do this in serial, 
...
static List* SyncBufferParellel(int buf_id, bool skip_recently_used, WritebackContext *wb_context, int * sync_result)
{
    ...
    ret = CheckBufferNeedSync(buf_id, skip_recently_used, &bufstatus);
    if (SYNC_BUF_KEEP_GOING == ret)
    {
        ret = SyncBufferPrePhase1(buf_id);
        if (SYNC_BUF_KEEP_GOING == ret)
        {
            /*
             * Setup error traceback support for ereport()
             * keep this as orginal, while, it just traces the current buf_id. 
             * to remember, there are buff_ids in crypt queues.
             */
            errcallback.callback = shared_buffer_write_error_callback;
            errcallback.arg      = (void *) GetBufferDescriptor(buf_id);
            errcallback.previous = error_context_stack;
            error_context_stack  = &errcallback;

            /* flush xlog */
            SyncBufferPrePhase2(buf_id);

            buf_id_list = SyncBufferDoing(buf_id, bufstatus);

            /* write the buffer(crypt or normal), and terminateIO */
        ...
        else if (SYNC_BUF_BREAK == ret)
        {
            /* 
             * fail to start buf io 
             */
...
char * BufHdrGetBlockFunc(BufferDesc * buf)
{
    return BufHdrGetBlock(buf);
}
