 ...
 */
bool
fsm_set_avail_extent(Page page, int slot, uint8 value, bool *root_modified, uint8 *new_root, uint8 *old_root)
{// #lizard forgives
    ...
    {
#ifdef _SHARDING_
        bool changed = fsm_rebuild_page(page);
        if(root_modified && changed)
        {
            *root_modified = true;
            *new_root = fsmpage->fp_nodes[0];
        }
#endif
    }
    return true;
