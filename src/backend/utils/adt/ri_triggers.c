...
{// #lizard forgives
    ...
    {
        Relation    tg_relation = trigdata->tg_relation;

        if (tg_relation && tg_relation->rd_locator_info &&
            tg_relation->rd_locator_info->locatorType == LOCATOR_TYPE_SHARD)
        {
            /* shard table */
        }
        else
...
{// #lizard forgives
    ...
    switch (riinfo->confmatchtype)
    {
        ...
        case FKCONSTR_MATCH_SIMPLE:
        case FKCONSTR_MATCH_FULL:
            switch (ri_NullCheck(RelationGetDescr(fk_rel), old_row, riinfo, true))
            {
                case RI_KEYS_ALL_NULL:
                case RI_KEYS_SOME_NULL:
...
{// #lizard forgives
    ...
    if (SPI_processed > 0)
    {
        ...
        if (fake_riinfo.confmatchtype == FKCONSTR_MATCH_FULL &&
            ri_NullCheck(RelationGetDescr(fk_rel), tuple, &fake_riinfo, false) != RI_KEYS_NONE_NULL)
            ereport(ERROR,
                    (errcode(ERRCODE_FOREIGN_KEY_VIOLATION),
                     errmsg("insert or update on table \"%s\" violates foreign key constraint \"%s\"",
                            RelationGetRelationName(fk_rel),
...
#ifdef _MLS_ 
static int
ri_NullCheck(TupleDesc tupDesc, HeapTuple tup,
             const RI_ConstraintInfo *riinfo, bool rel_is_pk)
#endif             
{
    const int16 *attnums;
