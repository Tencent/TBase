...
AllocateRelationDesc(Form_pg_class relp)
{
    ...
    {
        RelationIdCacheLookup(relationForm->relparent, parent);        
        if (!RelationIsValid(parent))
        {
            /* no parent found, create it first */
            parent = RelationBuildDesc(relationForm->relparent, true);    
        }
    }
    else
    {
...
#ifdef _MLS_
    //cls_assgin_relation_tupledesc_column(relation);
    dmask_assgin_relat_tupledesc_fld(relation);
    trsprt_crypt_asign_rela_tuldsc_fld(relation);
#endif
...
RelationBuildPartitionInfo(Relation rel)
{
    ...
    Form_pg_partition_interval pg_partition_parent;
    ...
    prel = heap_open(PgPartitionIntervalRelationId, AccessShareLock);
    pcscan = systable_beginscan(prel, PgPartitionIntervalRelIndexId, true,
                                NULL, 1, &skey);
    htup = systable_getnext(pcscan);

    if (!HeapTupleIsValid(htup))
    {
        rel->rd_partitions_info = NULL;
    }
    else
    {        
        oldContext = MemoryContextSwitchTo(CacheMemoryContext);
        htup = heap_copytuple(htup);
        pg_partition_parent = (Form_pg_partition_interval) GETSTRUCT(htup);
        
        rel->rd_partitions_info = (Form_pg_partition_interval) palloc(sizeof(FormData_pg_partition_interval));
        
        memcpy(rel->rd_partitions_info, pg_partition_parent, sizeof(FormData_pg_partition_interval));        
...
{// #lizard forgives
    ...
    if (RELATION_IS_CHILD(relation))
    {
        RelationIdCacheLookup(relation->rd_rel->relparent, parent);        
        if (!RelationIsValid(parent))
        {
            /* no parent found, create it first */
            elog(ERROR, "child table %u find relcache parent %u failed",
                 targetRelId, relation->rd_rel->relparent);
        }
        
        /* child tuple desc just point to parent's, and increase the reference count */
        relation->rd_att = parent->rd_att;
        relation->rd_att->tdrefcount++;
        relation->rd_rel->relnatts = parent->rd_rel->relnatts;
    }
    else
    {
#endif
...
#ifdef _MLS_
    mls_create_cls_check_expr(relation);
#endif
...
#ifdef _MLS_
    if (relation->rd_cls_struct)
    {
        MemoryContextDelete(relation->rd_cls_struct->mctx);
        relation->rd_cls_struct = NULL;
    }
#endif
