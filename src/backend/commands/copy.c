...
{// #lizard forgives
    ...
    if (is_from)
    {
        ...
        if ((XactReadOnly  || GTM_ReadOnly) && (rel && !rel->rd_islocaltemp))
...
{// #lizard forgives
    ...
    if (cstate->rel)
    {
        ...
        {
            diskey = RelationGetDisKey(cstate->rel);

            if(diskey > RelationGetDescr(cstate->rel)->natts)
            {
                elog(ERROR, "AttrNum[%d] of distribute key is invalid, ",diskey);
            }

            secdiskey = RelationGetSecDisKey(cstate->rel);
        }
#endif
#ifdef _MLS_
        ...
        /* open partitions */
        if(RELATION_IS_INTERVAL(cstate->rel))
        {
#ifdef _MLS_            
            parent_oid = RelationGetRelid(cstate->rel);
#endif
            /* skip patitions by cold data strategy */
            nchildren = RelationGetNParts(cstate->rel);
            children  = bms_init_set(nchildren);
#ifdef __COLD_HOT__
            PruneHotData(cstate->rel->rd_id, children);
#endif

            cstate->partrels = (Relation *)palloc(nchildren * sizeof(Relation));
            cstate->nparts = 0;
            for(i = 0; i < nchildren; i++)
            {
                if (bms_is_member(i, children))
                {
        ...
        if(cstate->nparts > 0)
        {
            ...
            for(i = 0; i < cstate->nparts; i++)
            {
                scandesc = heap_beginscan(cstate->partrels[i], GetActiveSnapshot(), 0, NULL);
                while ((tuple = heap_getnext(scandesc, ForwardScanDirection)) != NULL)
                {
                    bool isdeformed = false;
                    CHECK_FOR_INTERRUPTS();

                    /* this tuple is belong shard group?*/
                    if(cstate->shard_array > 0)
                    {
                        shardid = (int)HeapTupleGetShardId(tuple);                    
                                    
                        if(shardid >= 0 && !bms_is_member(shardid, cstate->shard_array))
                            continue;
                        ...
                        if(!ShardIDIsValid(shardid) && RelationIsSharded(cstate->rel))
                        {
                            ...
                            Datum secValue;
                            bool  issecnull;
                            Oid   typeOfSecDistCol;
                            
                            //elog(INFO, "[shardiderror] shardid cannot be invalid in shard tables %s.", RelationGetRelationName(cstate->partrels[i]));
#ifdef _MLS_
                            if (NULL != cstate->rel->rd_att->transp_crypt)
                            {
                            	trsprt_crypt_dcrpt_all_col_vale_cp(tuple, cstate->rel->rd_att, values, nulls);
                            }
                            else
                            {
#endif                                
                            ...
                            if (diskey != InvalidAttrNumber) 
                            {
                                typeOfDistCol = RelationGetDescr(cstate->rel)->attrs[diskey - 1]->atttypid;
                                value          = values[diskey - 1];
                                isdisnull      = nulls[diskey - 1];
                            }
                            else
                            {
                                typeOfDistCol = InvalidOid;
                                value          = 0;
                                isdisnull      = true;
                            }
                            ...
                            {
                                typeOfSecDistCol = RelationGetDescr(cstate->rel)->attrs[secdiskey - 1]->atttypid;
                                secValue         = values[secdiskey - 1];
                                issecnull         = nulls[secdiskey - 1];
                            }
                            else
                            {
                                typeOfSecDistCol = InvalidOid;
                                secValue          = 0;
                                issecnull      = true;
                            }
                            ...
                            shardid = EvaluateShardId(typeOfDistCol, isdisnull, value,
                                                      typeOfSecDistCol, issecnull, secValue, RelationGetRelid(cstate->rel));
                            //HeapTupleHeaderSetShardId(tuple, shardId);        
                    ...
                    if (has_datamask)
                    {
                        dmask_exchg_all_cols_value_copy(cstate->rel->rd_att, values, nulls, parent_oid);
                    }
                    if (InvalidAttrNumber != cls_attnum)
                    {
                        if (!mls_cls_check_row_validation_in_cp(values[cls_attnum -1 ]))
                        {
                            continue;
                        }
                    }
            ...
            for(i = 0; i < cstate->nparts; i++)
            {
                heap_close(cstate->partrels[i], AccessShareLock);
            }
            
            pfree(cstate->partrels);
            cstate->partrels = NULL;
        }
        else
        {
...
#define MAX_BUFFERED_TUPLES 1000
    ...
    Size        *part_bufferedTuplesSize = NULL;
    ...
    if(IS_PGXC_DATANODE && RELATION_IS_INTERVAL(cstate->rel))
    {
        ...
        resultRelInfo->partpruning = RelationGetPartitionsByQuals(cstate->rel,NULL);
    }
    else
    {
        resultRelInfo->ispartparent = false;
        resultRelInfo->partpruning = NULL;
    }
#endif
    ...
    else
    {
        ...
        {
            int i;
            part_bufferedTuples = (HeapTuple **)palloc0(RelationGetNParts(cstate->rel)*sizeof(HeapTuple*));
            for(i = 0; i < RelationGetNParts(cstate->rel); i++)
            {
                part_bufferedTuples[i] = (HeapTuple *)palloc(MAX_BUFFERED_TUPLES * sizeof(HeapTuple));
            }

            part_bufferedTuplesSize = (Size *)palloc0(RelationGetNParts(cstate->rel) * sizeof(Size));
            part_nBufferedTuples = (int *)palloc0(RelationGetNParts(cstate->rel) * sizeof(int));

            npart = RelationGetNParts(cstate->rel);
        }
#endif
    ...
    {
        int i = 0;
        part_bistates = (BulkInsertState *)palloc(RelationGetNParts(cstate->rel) * sizeof(BulkInsertState));
        for(i = 0; i < RelationGetNParts(cstate->rel); i++)
        {
            part_bistates[i] = GetBulkInsertState_part(npart);
        }
        /* for default partition */
        bistate = GetBulkInsertState_part(npart);
    }
    else
    {
#endif
...
readnextline:            
        ...
        if (IS_PGXC_COORDINATOR && cstate->remoteCopyState->rel_loc)
        {
...
#ifdef __COLD_HOT__
            if (AttributeNumberIsValid(rcstate->rel_loc->secAttrNum))
            {
                secValue = values[rcstate->rel_loc->secAttrNum - 1];
                secisnull = nulls[rcstate->rel_loc->secAttrNum - 1];
            }
#endif
            ...
            if (DataNodeCopyIn(cstate->line_buf.data,
                               cstate->line_buf.len,
#ifdef __COLD_HOT__
                               GET_NODES(rcstate->locator, value, isnull, secValue, secisnull, NULL),
#else
...
#endif
            {
                ...
                initStringInfo(sql);
#ifdef __COLD_HOT__
                conn_count = GET_NODES(rcstate->locator, value, isnull, secValue, secisnull, NULL);
#else
                conn_count = GET_NODES(rcstate->locator, value, isnull, NULL);
#endif
        ...
        if(RelationIsSharded(cstate->rel))
        {            
            tuple = heap_form_tuple_plain(tupDesc,values,nulls,RelationGetDisKey(cstate->rel),
                                          RelationGetSecDisKey(cstate->rel), RelationGetRelid(cstate->rel));
        ...
        if (!skip_tuple)
        {
            ...
            else
            {
				...
				* partition.
                ...
                {
                    /* router for tuple */
                    partkey = RelationGetPartitionColumnIndex(cstate->rel);
                    ...
                    {
                        partidx = RelationGetPartitionIdxByValue(cstate->rel,partvalue);
                    }
                }                        
#endif
                if (useHeapMultiInsert)
                {
                    ...
                    if(IS_PGXC_DATANODE && RELATION_IS_INTERVAL(cstate->rel) && partidx >= 0)
                    {
                        ...
                        part_bufferedTuplesSize[partidx] += tuple->t_len;
                    }
                    else
                    {
#endif
                    ...
                    if(IS_PGXC_DATANODE && RELATION_IS_INTERVAL(cstate->rel))
                    {
                        if(part_nBufferedTuples[partidx] == MAX_BUFFERED_TUPLES ||
                            part_bufferedTuplesSize[partidx] > 65535 ||
                            nBufferedTuples == MAX_BUFFERED_TUPLES ||
                ...
                else
                {
                    ...
                    if(IS_PGXC_DATANODE && RELATION_IS_INTERVAL(cstate->rel) && partidx >= 0)
                    {
                        ...
                        heap_insert(partRel->ri_RelationDesc, tuple,
                                                mycid, hi_options, part_bistates[partidx]);
                        
                        if (partRel->ri_NumIndices > 0)
                        {
                            tempRelInfo =  estate->es_result_relation_info;
                            estate->es_result_relation_info = partRel;
    ...
    if(IS_PGXC_DATANODE && RelationGetNParts(cstate->rel)> 0)
    {
        int i = 0;
        for(i = 0; i < RelationGetNParts(cstate->rel); i++)
            FreeBulkInsertState(part_bistates[i]);
    }
#endif
    ...
    if(IS_PGXC_DATANODE && npart > 0)
    {
        if(part_nBufferedTuples)
            pfree(part_nBufferedTuples);
        if(part_bufferedTuplesSize)
            pfree(part_bufferedTuplesSize);

        if(part_bufferedTuples)
        {
            int ii;
            for(ii=0; ii < npart; ii++)
            {
                if(part_bufferedTuples[ii])
                    pfree(part_bufferedTuples[ii]);
            }
            pfree(part_bufferedTuples);
        }
    }
#endif
    ...
    if(resultRelInfo->ispartparent)
    {
        int j=0;
        ResultRelInfo *partrel = NULL;
        for(j = 0; j < resultRelInfo->partarraysize; j++)
        {
