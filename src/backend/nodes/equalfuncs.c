...
_equalAlterNodeStmt(const AlterNodeStmt *a, const AlterNodeStmt *b)
{
    ...
    COMPARE_STRING_FIELD(cluster_name);
    COMPARE_NODE_FIELD(options);
    return true;
}
...
#ifdef __AUDIT__
static bool
_equalAuditStmt(const AuditStmt *a, const AuditStmt *b)
{
    COMPARE_SCALAR_FIELD(audit_ison);
    COMPARE_SCALAR_FIELD(audit_type);
    COMPARE_SCALAR_FIELD(audit_mode);
    COMPARE_NODE_FIELD(action_list);
    COMPARE_NODE_FIELD(user_list);
    COMPARE_NODE_FIELD(object_name);
    COMPARE_SCALAR_FIELD(object_type);

    return true;
}

static bool
_equalCleanAuditStmt(const CleanAuditStmt *a, const CleanAuditStmt *b)
{
    COMPARE_SCALAR_FIELD(clean_type);
    COMPARE_NODE_FIELD(user_list);
    COMPARE_NODE_FIELD(object_name);
    COMPARE_SCALAR_FIELD(object_type);
...
#ifdef __COLD_HOT__
static bool
_equalCheckOverLapStmt(const CheckOverLapStmt *a, const CheckOverLapStmt *b)
{
    COMPARE_NODE_FIELD(first);
    COMPARE_NODE_FIELD(second);
    return true;
}
...
#ifdef __AUDIT__
        case T_AuditStmt:
            retval = _equalAuditStmt(a, b);
            break;
        case T_CleanAuditStmt:
            retval = _equalCleanAuditStmt(a, b);
            break;
#endif
#ifdef __COLD_HOT__
