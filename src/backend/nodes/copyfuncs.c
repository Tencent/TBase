...
_copyPlannedStmt(const PlannedStmt *from)
{
    ...
    COPY_SCALAR_FIELD(need_snapshot);
#endif

#ifdef __AUDIT__
    COPY_STRING_FIELD(queryString);
    COPY_NODE_FIELD(parseTree);
#endif
...
#ifdef __AUDIT_FGA__
    COPY_NODE_FIELD(audit_fga_quals);
#endif
}
...
_copyModifyTable(const ModifyTable *from)
{
    ...
    COPY_SCALAR_FIELD(parentplanidx);
    COPY_NODE_FIELD(partplans);
    COPY_BITMAPSET_FIELD(partpruning);
#endif
...
_copyColumnDef(const ColumnDef *from)
{
    ...
    {
        COPY_POINTER_FIELD(ptr, ATTRIBUTE_FIXED_PART_SIZE);
    }
#endif
    return newnode;
}
...
#ifdef _SHARDING_
static VacuumShardStmt *
_copyVacuumShardStmt(const VacuumShardStmt *from)
{
    VacuumShardStmt *newnode = makeNode(VacuumShardStmt);
    COPY_SCALAR_FIELD(options);
    COPY_NODE_FIELD(relation);
    COPY_NODE_FIELD(shards);
...
{
    AddDropPartitions *newnode = makeNode(AddDropPartitions);
...
_copyPartitionForExpr(const PartitionForExpr *from)
{
    PartitionForExpr *newnode= makeNode(PartitionForExpr);
    COPY_SCALAR_FIELD(isdefault);
    COPY_NODE_FIELD(router_src);

    return newnode;
}

static ExchangeIndexName*
_copyExchangeIndexName(const ExchangeIndexName *from)
{
    ExchangeIndexName *newnode = makeNode(ExchangeIndexName);

    COPY_NODE_FIELD(oldIndexRelation);
    COPY_NODE_FIELD(newIndexRelation);
    COPY_SCALAR_FIELD(oldIndexId);
    COPY_SCALAR_FIELD(newIndexId);

    return newnode;
}

static ModifyPartStartValue*
_copyModifyPartStartValue(const ModifyPartStartValue *from)
{
    ModifyPartStartValue *newnode= makeNode(ModifyPartStartValue);
...
_copyAuditFgaStmt(const AuditFgaPolicy *from)
{
    AuditFgaPolicy *newnode= makeNode(AuditFgaPolicy);
...
#ifdef __AUDIT__
static AuditStmt *
_copyAuditStmt(const AuditStmt *from)
{
    AuditStmt *newnode = makeNode(AuditStmt);

    COPY_SCALAR_FIELD(audit_ison);
    COPY_SCALAR_FIELD(audit_type);
    COPY_SCALAR_FIELD(audit_mode);
    COPY_NODE_FIELD(action_list);
    COPY_NODE_FIELD(user_list);
    COPY_NODE_FIELD(object_name);
    COPY_SCALAR_FIELD(object_type);

    return newnode;
}

static CleanAuditStmt *
_copyCleanAuditStmt(const CleanAuditStmt *from)
{
    CleanAuditStmt *newnode = makeNode(CleanAuditStmt);

    COPY_SCALAR_FIELD(clean_type);
    COPY_NODE_FIELD(user_list);
    COPY_NODE_FIELD(object_name);
    COPY_SCALAR_FIELD(object_type);
...
#ifdef __COLD_HOT__
static CreateKeyValuesStmt *
_copyCreateKeyValuesStmt(const CreateKeyValuesStmt *from)
{
    CreateKeyValuesStmt *newnode = makeNode(CreateKeyValuesStmt);
    COPY_NODE_FIELD(valuesLists);
    COPY_NODE_FIELD(relinfo);
    COPY_STRING_FIELD(group);
    COPY_STRING_FIELD(coldgroup);

    return newnode;
}

static CheckOverLapStmt *
_copyCheckOverLapStmt(const CheckOverLapStmt *from)
{
    CheckOverLapStmt *newnode = makeNode(CheckOverLapStmt);

    COPY_NODE_FIELD(first);
    COPY_NODE_FIELD(second);
    return newnode;
}
...
{// #lizard forgives
    ...
    switch (nodeTag(from))
    {
        ...
        case T_AddDropPartitions:
            ...
            break;
        case T_PartitionForExpr:
            retval = _copyPartitionForExpr(from);
            break;
        case T_ExchangeIndexName:
            retval = _copyExchangeIndexName(from);
            break;
        case T_ModifyPartStartValue:
            retval = _copyModifyPartStartValue(from);
            break;
#endif

#ifdef __AUDIT_FGA__
        case T_AuditFgaPolicy:
            retval = _copyAuditFgaStmt(from);
            break; 
#endif
            /*
             * MISCELLANEOUS NODES
...
#ifdef __AUDIT__
        case T_AuditStmt:
            retval = _copyAuditStmt(from);
            break;
        case T_CleanAuditStmt:
            retval = _copyCleanAuditStmt(from);
            break;
#endif

#ifdef __COLD_HOT__
        case T_CreateKeyValuesStmt:
            retval = _copyCreateKeyValuesStmt(from);
            break;
