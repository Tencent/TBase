...
#ifdef __STORAGE_SCALABLE__
        if (subname && OidIsValid(subid))
        {
            appendStringInfo(&cmd, " SUBSCRIPTION_NAME \"%s\"", subname);
            appendStringInfo(&cmd, " ID_SUBSCRIPTION %u ", subid);
        }

        if (namespace && relname)
        {
            appendStringInfo(&cmd, " REL_NAMESPACE \"%s\"", namespace);
            appendStringInfo(&cmd, " NAME_REL \"%s\"", relname);
        }
#endif
    }
