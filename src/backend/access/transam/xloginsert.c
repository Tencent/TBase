...
{// #lizard forgives
    ...
    if (wal_is_cluster_stream())
    {
        info |= XLR_CLUSTER_STREAM;
    }
    else if (wal_is_internal_stream())
    {
        info |= XLR_INTERNAL_STREAM;
    }
    else
    {
        Assert(wal_is_user_stream());
    }
#endif
    ...
    *fpw_lsn = InvalidXLogRecPtr;
    for (block_id = 0; block_id < max_registered_block_id; block_id++)
    {
        ...
        bkpb.data_length = 0;
#ifdef _SHARDING_
        bkpb.sid = PageGetShardId(regbuf->page);
#endif
...
#ifdef _SHARDING_
                bool        no_hole = ((((PageHeader) page)->pd_flags & PD_NO_HOLE) > 0);
#endif
                if (!no_hole &&
