...
{// #lizard forgives
     ...
     */
    if (IsConnFromDatanode() && TransactIdIsCurentGlobalTransacId(xid))
        return true;
...
{// #lizard forgives
    ...
    bool        wrote_xlog;
#ifdef __SUPPORT_DISTRIBUTED_TRANSACTION__
    GlobalTimestamp global_committs = LocalCommitTimestamp;
#endif
    if(enable_distri_print)
    {
    ...
    else
    {
        ...
        if(IsPostmasterEnvironment && IS_PGXC_DATANODE && !IsLogicalWorker() && !AmTbaseSubscriptionApplyWorker()) 
        {
            ...
            pg_atomic_write_u64(&pgxact->prepare_timestamp, FrozenGlobalTimestamp);
            if(delay_before_acquire_committs)
            {
                pg_usleep(delay_before_acquire_committs);
            }
            /* use local commit ts instead of global if local commit */
            ...
            if(enable_distri_print)
            {
                elog(LOG, "local commit procno %d xid %d, time " INT64_FORMAT, MyProc->pgprocno, xid, global_committs);
            }    
        ...
        forceSyncCommit || nrels > 0)
    {
        ...
        if (markXidCommitted)
        {
            ...
            if(enable_distri_print)
            {
                elog(LOG, "commit transaction xid %d ts "INT64_FORMAT, xid, global_committs);
            }
        }
    }
    else
    {
        ...
        if (markXidCommitted)
        {
            ...
            TransactionTreeSetCommitTsData(xid, nchildren, children,
                                   global_committs,
                                   replorigin_session_origin_timestamp,
                                   replorigin_session_origin, false, XactLastRecEnd);
#endif
            if(enable_distri_print)
            {
                elog(LOG, "async commit transaction xid %d ts "INT64_FORMAT, xid, global_committs);
            }
        }
    }
...
#ifdef __SUPPORT_DISTRIBUTED_TRANSACTION__
    global_timestamp = GetGlobalTimestampGTM();
...
{// #lizard forgives
    ...
    /* Prevent cancel/die interrupt while CommitTransaction */
    ...
    if (!is_parallel_worker)
    {
        if (!IsConnFromDatanode())
        {
            ...
            if (TransactionIdIsValid(xid))
            {
                if(IsPostmasterEnvironment && IS_PGXC_DATANODE && !IsLogicalWorker()) 
                {
                    CheckGTMConnection();
                }
            }
...
#if 0
            if(enableTwoPhaseClean)
            {
                elog(LOG, "2pc txn gid: %s, startnode: %s, startxid: %u, participants: %s", prepareGID, MyProc->startnode, MyProc->startXid, MyProc->participants);
                record_2pc_involved_nodes_xid(prepareGID, MyProc->startnode, MyProc->startXid, MyProc->participants, s->transactionId);
            }
#endif
        }
        else
...
PushTransaction(void)
{
    ...
    if (InPlpgsqlFunc())
    {
        ...
        {
            elog(LOG, "[PLPGSQL] PushTransaction set need_begin_sub_txn to true");
        }
        s->need_send_begin_subtxn = true;
    }
#endif
...
SerializeTransactionState(Size maxsize, char *start_address)
{
    ...
    elog(LOG, "serialize next xid %u", GetNextTransactionId());    
    #endif
 ...
 */
XLogRecPtr
XactLogCommitRecord(TimestampTz global_timestamp,
 ...
 */
XLogRecPtr
XactLogAbortRecord(TimestampTz global_timestamp,
...
#ifdef __SUPPORT_DISTRIBUTED_TRANSACTION__
    global_timestamp = parsed->global_timestamp;
    if(!GlobalTimestampIsValid(global_timestamp))
        elog(ERROR, "commit timestamp is invalid for xid %d", xid);

    LWLockAcquire(CommitTsLock, LW_EXCLUSIVE);
    if(global_timestamp > ShmemVariableCache->latestCommitTs)
    {
        ShmemVariableCache->latestCommitTs = global_timestamp;
    }

    if(global_timestamp > ShmemVariableCache->latestGTS)
    {
        ShmemVariableCache->latestGTS = global_timestamp;
    }
...
xact_redo_abort(xl_xact_parsed_abort *parsed, TransactionId xid)
{
    ...
    LWLockAcquire(CommitTsLock, LW_EXCLUSIVE);
    if(parsed->global_timestamp > ShmemVariableCache->latestCommitTs)
    {
        ShmemVariableCache->latestCommitTs = parsed->global_timestamp;
    }

    if(parsed->global_timestamp > ShmemVariableCache->latestGTS)
    {
        ShmemVariableCache->latestGTS = parsed->global_timestamp;
    }
    LWLockRelease(CommitTsLock);
...
{// #lizard forgives
    ...
    else if (info == XLOG_XACT_PREPARE)
    {
        ...
        LWLockAcquire(TwoPhaseStateLock, LW_EXCLUSIVE);
#if 0
        record_2pc_redo_gid_xid(XLogRecGetData(record),
                                record->ReadRecPtr,
                                record->EndRecPtr);
#endif
    ...
    else if (info == XLOG_XACT_ACQUIRE_GTS)
    {
        ...
        LWLockAcquire(CommitTsLock, LW_EXCLUSIVE);
        if(xlrec->global_timestamp > ShmemVariableCache->latestGTS)
        {
            ShmemVariableCache->latestGTS = xlrec->global_timestamp;
        }
        LWLockRelease(CommitTsLock);
    }
#endif
