...
#ifdef _MLS_
static void mls_forbid_update_rolpassword(Relation rel, HeapTuple oldtup, HeapTuple newtup);
bool g_mls_update_rolpassword_by_alter = false;
...
get_next_page:
...
#ifdef _SHARDING_
        {
            ...
            if(PageIsNew(dp))
            {
                ...
                && RelationHasExtent(scan->rs_rd)
                && IsMVCCSnapshot(scan->rs_snapshot))
            {
                Snapshot snapshot = scan->rs_snapshot;
                ShardID pg_sid = PageGetShardId(dp);
                bool shard_is_visible = bms_is_member(pg_sid/snapshot->groupsize,
            ...
            if(to_skip)
            {
                ...
                {
                    if(RelationHasExtent(scan->rs_rd))
                        page = (page / PAGES_PER_EXTENTS + 1) * PAGES_PER_EXTENTS;
                    else
                        page++;
                ...
                else if((ScanDirectionIsBackward(dir)))
                {
                    ...
                    if(!finished && RelationHasExtent(scan->rs_rd))
                    {
                        if(page < PAGES_PER_EXTENTS)
                        {
                            finished = true;
                            page = scan->rs_nblocks - 1;
                        }
                        ...
                        {
                            page = (page / PAGES_PER_EXTENTS) * PAGES_PER_EXTENTS - 1;
                            finished = (page <= scan->rs_startblock);
                        }
                    }
                    else
...
get_next_page:
        ...
        if(RelationHasExtent(scan->rs_rd))
        {
            ...
            if(to_skip)
            {
                ...
                {
                    BlockNumber oldpage = page;
                    page = (page / PAGES_PER_EXTENTS + 1) * PAGES_PER_EXTENTS;
                    scan->rs_numblocks = scan->rs_numblocks - (page - oldpage);

                    if (page >= scan->rs_nblocks)
                        page = 0;
                    finished = (oldpage < scan->rs_startblock && page >= scan->rs_startblock)
                ...
                else if((ScanDirectionIsBackward(dir)))
                {
                    ...
                    {
                        page = scan->rs_nblocks - 1;
                        scan->rs_numblocks = scan->rs_numblocks - page;
                    }
                    else
                    {
                        page = (page / PAGES_PER_EXTENTS) * PAGES_PER_EXTENTS - 1;
                        scan->rs_numblocks = scan->rs_numblocks - (oldpage - page);
                    }
                        ...
                        bool temp_snap)
{
    ...
    if(enable_distri_debug)
    {
        scan->rs_scan_number = 0;
        scan->rs_valid_number = 0;
        scan->rs_invalid_number = 0;
        scan->rs_scan_start_timestamp = GetCurrentTimestamp();
        scan->rs_snapshot->number_visible_tuples = 0;
        scan->rs_snapshot->scanned_tuples_before_prepare = 0;
        scan->rs_snapshot->scanned_tuples_after_prepare = 0;
        scan->rs_snapshot->scanned_tuples_after_committed = 0;
        scan->rs_snapshot->scanned_tuples_after_abort = 0;
    }
#endif
...
{// #lizard forgives
    ...
    if(enable_distri_debug)
    {
        ...
        {
            elog(LOG, "statistic account: rel %s snapshot start ts " INT64_FORMAT " scanned number " INT64_FORMAT,
                    RelationGetRelationName(scan->rs_rd), scan->rs_snapshot->start_ts, scan->rs_scan_number);
            elog(LOG, "statistic account: rel %s snapshot start ts " INT64_FORMAT " valid scanned number " INT64_FORMAT,
                    RelationGetRelationName(scan->rs_rd), scan->rs_snapshot->start_ts, scan->rs_valid_number);
            elog(LOG, "statistic account: rel %s snapshot start ts " INT64_FORMAT " non valid scanned number " INT64_FORMAT,
                    RelationGetRelationName(scan->rs_rd), scan->rs_snapshot->start_ts, scan->rs_invalid_number);
            elog(LOG, "statistic account: rel %s snapshot start ts " INT64_FORMAT " snapshot scanned number " INT64_FORMAT,
                    RelationGetRelationName(scan->rs_rd), scan->rs_snapshot->start_ts, scan->rs_snapshot->number_visible_tuples);
        }
        LogScanGTM(GetTopTransactionIdIfAny(), 
                       PGXCNodeName, 
                       scan->rs_snapshot->start_ts,
                       scan->rs_scan_start_timestamp,
                       GetCurrentTimestamp(),
                       SCAN,
        ...
        LogScanGTM(GetTopTransactionIdIfAny(), 
                       ...
                       SNAPSHOT_SCAN,
                       RelationGetRelationName(scan->rs_rd),
                       scan->rs_snapshot->number_visible_tuples);
        LogScanGTM(GetTopTransactionIdIfAny(), 
                       PGXCNodeName, 
                       scan->rs_snapshot->start_ts,
                       scan->rs_scan_start_timestamp,
                       ...
                       RelationGetRelationName(scan->rs_rd),
                       scan->rs_snapshot->scanned_tuples_before_prepare);
        LogScanGTM(GetTopTransactionIdIfAny(), 
                       PGXCNodeName, 
                       scan->rs_snapshot->start_ts,
                       scan->rs_scan_start_timestamp,
                       ...
                       RelationGetRelationName(scan->rs_rd),
                       scan->rs_snapshot->scanned_tuples_after_prepare);
        LogScanGTM(GetTopTransactionIdIfAny(), 
                       PGXCNodeName, 
                       scan->rs_snapshot->start_ts,
                       scan->rs_scan_start_timestamp,
                       ...
                       RelationGetRelationName(scan->rs_rd),
                       scan->rs_snapshot->scanned_tuples_after_committed);
        LogScanGTM(GetTopTransactionIdIfAny(), 
                       PGXCNodeName, 
                       scan->rs_snapshot->start_ts,
                       scan->rs_scan_start_timestamp,
                       ...
                       RelationGetRelationName(scan->rs_rd),
                       scan->rs_snapshot->scanned_tuples_after_abort);
    }
#endif
    /*
     * unpin scan buffers
...
GetBulkInsertState_part(int npart)
{
    ...
    bistate = (BulkInsertState) palloc(sizeof(BulkInsertStateData));
    bistate->strategy = GetAccessStrategy_part(npart);    
    bistate->current_buf = InvalidBuffer;
    return bistate;
}
#endif
...
{// #lizard forgives
    ...
    HeapTuple    heaptup;
#ifdef _MLS_
    HeapTuple    tuple_encrypted = NULL; 
    HeapTuple    tuple_kept      = NULL;
#endif
    Buffer        buffer;
...
#ifdef _MIGRATE_
    if(relation->rd_locator_info && relation->rd_locator_info->locatorType == LOCATOR_TYPE_SHARD 
            && !ShardIDIsValid(HeapTupleGetShardId(tup)))
    {
        elog(ERROR, "tuple has invalid shardid[%d]", HeapTupleGetShardId(tup));
    }
#endif

#ifdef _SHARDING_
    if(RelationHasExtent(relation) && IsShardBarriered(relation->rd_node, HeapTupleGetShardId(tup)))
    {
...
#ifdef _MLS_
    if (NULL != relation->rd_att->transp_crypt)
    {
        tuple_encrypted = transparent_crypt_encrypt_columns(relation, heaptup, MessageContext); 
        tuple_kept      = heaptup;
        heaptup         = tuple_encrypted;
    }
#endif
     ...
     */
    buffer = RelationGetBufferForTuple_shard(relation, HeapTupleGetShardId(tup),
                                        heaptup->t_len,
...
#ifdef _MIGRATE_
        xlhdr.t_shardid = heaptup->t_data->t_shardid;
#endif
    ...
    {
        UpdateShardStatistic(CMD_INSERT, heaptup->t_data->t_shardid, heaptup->t_len, 0);
    }
#endif
    ...
    {
        heaptup = tuple_kept;
        heaptup->t_self = tuple_encrypted->t_self;

        heap_freetuple(tuple_encrypted);
    }
#endif
...
{// #lizard forgives
    ...
    Size        saveFreeSpace;
#ifdef _SHARDING_
    ShardID        tuple_sid = InvalidShardID;
#endif
    ...
    if (NULL != relation->rd_att->transp_crypt)
    {
        tuple_encrypted = palloc(ntuples * sizeof(HeapTuple));
        tuple_kept = palloc(ntuples * sizeof(HeapTuple));
        for (i = 0; i < ntuples; i++)
        {
            tuple_encrypted[i] = transparent_crypt_encrypt_columns(relation, heaptuples[i], CurrentMemoryContext); 
            tuple_kept[i]      = heaptuples[i];
            heaptuples[i]      = tuple_encrypted[i];
        }
    }
#endif
    ...
    while (ndone < ntuples)
    {
        ...
        {
            tuple_sid = HeapTupleGetShardId(heaptuples[ndone]);
        }
        else
        {
            tuple_sid = InvalidShardID;
        }
#endif
         ...
         */
        buffer = RelationGetBufferForTuple_shard(relation, 
#ifdef _SHARDING_
                                            tuple_sid, 
#endif
...
#ifdef _SHARDING_
            /*
             * if this tuple has not same shardid as previous tuple, we need to switch block.
             */
            if(RelationHasExtent(relation)
                && HeapTupleGetShardId(heaptup) != PageGetShardId(page))
            {
                break;
            }
#endif
...
#ifdef _MIGRATE_
                tuphdr->t_shardid = heaptup->t_data->t_shardid;
#endif
    ...
    if (g_StatShardInfo && IS_PGXC_DATANODE)
    {
        for (i = 0; i < ntuples; i++)
        {
            UpdateShardStatistic(CMD_INSERT, heaptuples[i]->t_data->t_shardid, heaptuples[i]->t_len, 0);
        }
    }
#endif
    ...
    if (NULL != relation->rd_att->transp_crypt)
    {
        for (i = 0; i < ntuples; i++)
        {
            heaptuples[i] = tuple_kept[i];
            heaptuples[i]->t_self = tuple_encrypted[i]->t_self;
            
            heap_freetuple(tuple_encrypted[i]);
        }

        pfree(tuple_kept);
        pfree(tuple_encrypted);
    }
#endif
...
{// #lizard forgives
    ...
    page = BufferGetPage(buffer);
#ifdef _SHARDING_
    if(RelationHasExtent(relation) && IsShardBarriered(relation->rd_node, PageGetShardId(page)))
    {
        elog(ERROR, "shard is be vacuuming now, so it is forbidden to write.");
    }
#endif
...
l1:
    ...
    {
        GlobalTimestamp committs = 0;
        TransactionId xmax = 0, xmin;
        bool xmax_invalid = false, locked = false, multi = false, heap_combocid = false;
        Snapshot active_snap = GetActiveSnapshot();
        
        UnlockReleaseBuffer(buffer);
        if(tp.t_data->t_infomask & HEAP_XMAX_INVALID)
        {
            elog(WARNING, "xmax invalid");
            xmax_invalid = true;
        }

        if(HEAP_XMAX_IS_LOCKED_ONLY(tp.t_data->t_infomask))
        {
            elog(WARNING, "xmax locked only");
            locked = true;

        }
        if(tp.t_data->t_infomask & HEAP_COMBOCID)
        {
            elog(WARNING, "heap combocid");
            heap_combocid = true;
        }
        
        if(tp.t_data->t_infomask & HEAP_XMAX_IS_MULTI)
        {
            elog(WARNING, "xmax is multi");
            xmax = HeapTupleGetUpdateXid(tp.t_data);
            multi = true;
        }
        else
        {
            elog(WARNING, "normal xmax");
            xmax = HeapTupleHeaderGetRawXmax(tp.t_data);
        }
            
    
        xmin = HeapTupleHeaderGetRawXmin(tp.t_data);
        if(TransactionIdGetCommitTsData(xmin, &committs,  NULL))
        {
            elog(WARNING, "committed in tlog");
        }
        ereport(ERROR,
                (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                 errmsg("attempted to delete invisible tuple in rel %s xid %u xmin %u xmax %u is-current %d in-progress %d committted %d "
                        "cid %d tuple cmin %d cmax %d "
                        "active snapshot cid %d start_ts "INT64_FORMAT " local %d snap xmin %d getcurcid %d "
                        "tuple commit ts "INT64_FORMAT " tlog ts "INT64_FORMAT " xmax invalid %d locked %d multi %d heap combocid %d", 
                        ...
                        cid,
                        HeapTupleHeaderGetCmin(tp.t_data),
                        HeapTupleHeaderGetCmax(tp.t_data),
                        active_snap->curcid,
                        active_snap->start_ts,
                        active_snap->local,
                        active_snap->xmin,
                        GetCurrentCommandId(false),
						HeapTupleHderGetXminTimestapAtomic(tp.t_data),
                        committs,
                        xmax_invalid,
                        locked,
                        multi,
     ...
     */
#ifdef __SUPPORT_DISTRIBUTED_TRANSACTION__
    PageSetPrunable(page, xid, InvalidGlobalTimestamp);
#else
...
#ifdef __STORAGE_SCALABLE__
            tup = &tp;
            xlhdr.t_shardid = HeapTupleGetShardId(tup);
#endif
    ...
    if (g_StatShardInfo && IS_PGXC_DATANODE)
    {
        UpdateShardStatistic(CMD_DELETE, tp.t_data->t_shardid, 0, tp.t_len);
    }
#endif
...
#ifdef _SHARDING_
    if(RelationHasExtent(relation) && !ShardIDIsValid(HeapTupleGetShardId(newtup)))
    {
...
#ifdef _SHARDING_
    if(RelationHasExtent(relation) && IsShardBarriered(relation->rd_node, HeapTupleGetShardId(newtup)))
    {
        elog(ERROR, "shard is be vacuuming now, so it is forbidden to write.");
    }
#endif
...
#ifdef _MLS_
    mls_forbid_update_rolpassword(relation, &oldtup, newtup);
#endif
...
l2:
...
#ifdef _MLS_
    if ((false == need_toast) && (NULL != relation->rd_att->transp_crypt))
    {
        tuple_encrypted = transparent_crypt_encrypt_columns(relation, newtup, CurrentMemoryContext);
        tuple_kept      = newtup;
        newtup          = tuple_encrypted;
    }
#endif
    ...
    if (need_toast || newtupsize > pagefree)
    {
        ...
        if (need_toast)
        {
            ...
            if(NULL != relation->rd_att->transp_crypt)
            {
                tuple_encrypted = transparent_crypt_encrypt_columns(relation, heaptup, CurrentMemoryContext);
                tuple_kept      = heaptup;
                heaptup         = tuple_encrypted;         
            }
#endif
        ...
        if (newtupsize > pagefree)
        {
            ...
            newbuf = RelationGetBufferForTuple_shard(relation, HeapTupleGetShardId(heaptup),
                                                heaptup->t_len,
                                               buffer, 0, NULL,
    ...
    if (use_hot_update)
    {
        ...
        if (tuple_kept)
        {
            HeapTupleSetHeapOnly(tuple_kept);
        }
#endif
    }
    else
    {
        ...
        if (tuple_kept)
        {
            HeapTupleClearHeapOnly(tuple_kept);
        }
#endif        
    }
    ...
    if (g_StatShardInfo && IS_PGXC_DATANODE)
    {
        UpdateShardStatistic(CMD_UPDATE, heaptup->t_data->t_shardid, heaptup->t_len, oldtup.t_len);
    }
#endif
...
heap_abort_speculative(Relation relation, HeapTuple tuple)
{
    ...
    Assert(TransactionIdIsValid(RecentGlobalXmin));
#ifdef __SUPPORT_DISTRIBUTED_TRANSACTION__
    PageSetPrunable(page, RecentGlobalXmin, InvalidGlobalTimestamp);
#else
        ...
        else
        {
            ...
            if (TransactionIdPrecedes(xid, cutoff_xid))
            {
#ifdef __SUPPORT_DISTRIBUTED_TRANSACTION__
                {
                    ...
                    if(TransactionIdGetCommitTsData(xid, &committs, NULL))
                    {
                        ...
                        else
                        {
                            elog(ERROR, "tuple cannot be frozen now, please try later xid %u cutoff xid %u committs "
                                        INT64_FORMAT " RecentDataTs "INT64_FORMAT " RecentGlobalXmin %u RecentGlobalDataXmin %u", 
                                        xid, cutoff_xid, committs, RecentDataTs, RecentGlobalXmin, RecentGlobalDataXmin);
                            *flags |= FRM_RETURN_IS_XID;
                        }
                    
                    }
                    else /* As xid is below cutoff_xid, the transaction should be crashed or aborted */  
                    {
...
#ifdef __SUPPORT_DISTRIBUTED_TRANSACTION__
            {
                GlobalTimestamp committs = HeapTupleHderGetXminTimestapAtomic(tuple);
                ...
                if(!GlobalTimestampIsValid(committs))
                {
                    if(!TransactionIdGetCommitTsData(xid, &committs, NULL))
                    {
                        ...
                        (errcode(ERRCODE_INTERNAL_ERROR),
                                errmsg("freezing xid %d should have commit timestamp committed %d abort %d in-progress %d", 
                                        xid,
                                        TransactionIdDidCommit(xid),
...
#ifdef __SUPPORT_DISTRIBUTED_TRANSACTION__
    xlrec.pruneTs = pruneTs;
    xlrec.hasData = 1;
    #ifdef __PAGE_TIMESTAMP_LOG__
    {
...
#ifdef _MIGRATE_
    xlhdr.t_shardid = newtup->t_data->t_shardid;
#endif
...
#ifdef __STORAGE_SCALABLE__
        xlhdr_idx.t_shardid = HeapTupleGetShardId(oldtup);
#endif
...
{// #lizard forgives
    ...
    if (action == BLK_NEEDS_REDO)
    {
        ...
        Size        datalen;
#ifdef __SUPPORT_DISTRIBUTED_TRANSACTION__
        PageSetPrunableTs(page, xlrec->pruneTs);
        #ifdef __PAGE_TIMESTAMP_LOG__
        
            need_prune = xlrec->hasData;
        #endif
     ...
     */
#ifdef __SUPPORT_DISTRIBUTED_TRANSACTION__
    if ((action == BLK_NEEDS_REDO) && need_prune)
#else
    if (action == BLK_NEEDS_REDO)
#endif
...
{// #lizard forgives
    ...
    if (XLogReadBufferForRedo(record, 0, &buffer) == BLK_NEEDS_REDO)
    {
...
#ifdef __SUPPORT_DISTRIBUTED_TRANSACTION__
        PageSetPrunable(page, XLogRecGetXid(record), InvalidGlobalTimestamp);
#else
        ...
        {
            UpdateShardStatistic(CMD_DELETE, htup->t_shardid, 0, ItemIdGetLength(lp));
        }
#endif
    }
    if (BufferIsValid(buffer))
...
{// #lizard forgives
...
#ifdef _SHARDING_
        ShardID sid;
        XLogRecGetBlockShardID(record, 0, &sid);
#endif
    ...
    if (action == BLK_NEEDS_REDO)
    {
...
#ifdef _MIGRATE_
        htup->t_shardid = xlhdr.t_shardid;
#endif        
        ...
        if (g_StatShardInfo && IS_PGXC_DATANODE && i_am_standby)
        {
            UpdateShardStatistic(CMD_INSERT, xlhdr.t_shardid, newlen, 0);
        }
#endif
    }
    if (BufferIsValid(buffer))
...
{// #lizard forgives
    ...
    else if (XLogRecGetInfo(record) & XLOG_HEAP_INIT_PAGE)
    {
        ...
        PageInit_shard(page, BufferGetPageSize(nbuffer), 0, sid, false);
    ...
    if (newaction == BLK_NEEDS_REDO)
    {
        ...
        if (g_StatShardInfo && IS_PGXC_DATANODE && i_am_standby)
        {
            UpdateShardStatistic(CMD_UPDATE, xlhdr.t_shardid, newlen, oldtup.t_len);
        }
#endif
    }
...
void mls_enable_update_rolpassword(void)
{
    if (g_enable_data_mask)
    {
        g_mls_update_rolpassword_by_alter = true;
    }
    return;
}

void mls_disable_update_rolpassword(void)
{
    g_mls_update_rolpassword_by_alter = false;
    return;
}

/*
 * pasword of pg_authid should not be update directly, 
 * in case someone skip password authentication and changes himself to 'white list' user in datamast feature.
 */
static void mls_forbid_update_rolpassword(Relation rel, HeapTuple oldtup, HeapTuple newtup)
{
    if (g_enable_data_mask)
    {   
        if (AuthIdRelationId == RelationGetRelid(rel))
        {   
            if (!heap_tuple_attr_equals(RelationGetDescr(rel),
                                            Anum_pg_authid_rolpassword, oldtup, newtup))
            {
                if (false == g_mls_update_rolpassword_by_alter)
                {
                    elog(ERROR, "update rolpassword is forbidden");
                }
            }
        }
    }
