...
static int heap_prune_chain(Relation relation, Buffer buffer,
                 ...
                 PruneState *prstate);
#ifdef __SUPPORT_DISTRIBUTED_TRANSACTION__
static void heap_prune_record_prunable(PruneState *prstate, TransactionId xid, GlobalTimestamp ts);
#else
...
{// #lizard forgives
     ...
     */
#ifdef __SUPPORT_DISTRIBUTED_TRANSACTION__
    if (!PageIsPrunable(page, OldestXmin, RecentDataTs))
        return;
#else
...
#ifdef __SUPPORT_DISTRIBUTED_TRANSACTION__
    prstate.new_prune_ts = InvalidGlobalTimestamp;
#endif
    ...
    for (;;)
    {
        ...
        switch (HeapTupleSatisfiesVacuum(&tup, OldestXmin, buffer))
        {
            ...
            case HEAPTUPLE_RECENTLY_DEAD:
                ...
                {
                    committs = HeapTupleHderGetXmaxTimestapAtomic(htup);
                }
...
static void
#ifdef __SUPPORT_DISTRIBUTED_TRANSACTION__
heap_prune_record_prunable(PruneState *prstate, TransactionId xid, GlobalTimestamp ts)
#else
...
#ifdef __SUPPORT_DISTRIBUTED_TRANSACTION__
    if(!GlobalTimestampIsValid(ts))
        return;
    
    if (!GlobalTimestampIsValid(prstate->new_prune_ts) ||
        (ts < prstate->new_prune_xid))
