                   ...
                   bool use_wal)
{
    ...
    state->rs_blockno = InvalidBlockNumber;
#else
...
{// #lizard forgives
    ...
    {
#ifdef _SHARDING_
        ShardID pg_sid = PageGetShardId(page);
        ShardID tup_sid = HeapTupleGetShardId(tup);
#endif
        pageFreeSpace = PageGetHeapFreeSpace(page);

        if (len + saveFreeSpace > pageFreeSpace
#ifdef _SHARDING_
                || (pg_sid != tup_sid && RelationHasExtent(state->rs_old_rel))
#endif
            )
        {
    ...
    if (!state->rs_buffer_valid)
    {
        ...
        state->rs_buf = RelationGetBufferForTuple_shard(state->rs_new_rel, 
                                        ...
                                        NULL,
                                        NULL);
        Assert(BufferIsValid(state->rs_buf));
        //LockBuffer(state->rs_buf, LW_EXCLUSIVE);
        state->rs_buffer = BufferGetPage(state->rs_buf);
        state->rs_blockno = BufferGetBlockNumber(state->rs_buf);
...
{// #lizard forgives
    ...
    cutoff = ReplicaSlotsComputLogicalRstartLSN();
