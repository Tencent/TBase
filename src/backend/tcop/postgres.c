 ...
 */
static void 
ReleaseSessionMemory (int code, Datum arg)
{
...
{// #lizard forgives
    ...
    foreach(parsetree_item, parsetree_list)
    {
        ...
        if (xact_started)
        {
            AuditReadQueryList(query_string, querytree_list);
        }
#endif
            ...
            list_length(plantree_list) == 1)
        {
            ...
            if (parse->isMultiValues && !parse->hasUnshippableTriggers)
            {
                bool success;
                InsertStmt *insert_stmt = (InsertStmt*)parsetree->stmt;

                plantree_list = transformInsertValuesIntoCopyFrom(NULL, insert_stmt, &success, 
...
{// #lizard forgives
    ...
    if (psrc->insert_into)
    {
        ...
        if (IsA(parse, Query) && parse->commandType == CMD_INSERT)
        {
            if (psrc->raw_parse_tree && IsA(psrc->raw_parse_tree->stmt, InsertStmt))
            {
                ...
                if (ncolumns == 0 || numParams == 0 || numParams % ncolumns != 0)
                {
        ...
        if (psrc->insert_into)
        {
            ...
            data_list = (char ***)palloc0(sizeof(char **) * (numParams / ncolumns));
            for (i = 0; i < (numParams / ncolumns); i++)
            {
        ...
        for (paramno = 0; paramno < numParams; paramno++)
        {
            ...
            if (pformat == 0)    /* text mode */
            {
                ...
                if (psrc->insert_into)
                {
                    ...
                    {
                        old_top = MemoryContextSwitchTo(TopTransactionContext);
                        data_list[column_index][index - 1] = pstrdup(pstring);
                        MemoryContextSwitchTo(old_top);
            ...
            else if (pformat == 1)    /* binary mode */
            {
                ...
                if (psrc->insert_into)
                {
                    ...
                    else
                    {
                        ...
                        data_list[column_index][index - 1] = pstring;
    ...
    if (psrc->insert_into)
    {
        ...
        {
            elog(ERROR, "number of datarows mismatched, expected %d, result %d",
                         numParams / ncolumns, column_index);
        }
    }
#endif
    ...
    if (psrc->insert_into && data_list)
    {
        ...
        insert_stmt->ndatarows = column_index;

        cplan->stmt_list = transformInsertValuesIntoCopyFrom(NULL, (InsertStmt*)psrc->raw_parse_tree->stmt, &success, 
                                                             copy_filename, parse);
...
#endif
#ifdef __COLD_HOT__
    /* free memory of g_TempKeyValueList */
    on_proc_exit(ReleaseSessionMemory, 0);
#endif
    /*
    ...
    for (;;)
    {
        ...
        switch (firstchar)
        {
            ...
            case 'N':
                {
                    ...
                    cursor = pq_getmsgstring(&input_message);
                    cons = pq_getmsgint(&input_message, 4);
                    pq_getmsgend(&input_message);

                    SetDisConnectConsumer(cursor, cons);
                    
                    SharedQueueDisconnectConsumer(cursor);

                    DropPreparedStatement(cursor, false);

                    if (whereToSendOutput == DestRemote)
                    {
                        pq_putmessage('C', NULL, 0);
                        pq_flush();
                    }
                }
                break;
            ...
            case 'g':            /* gxid */
                {
                    ...
                    pq_getmsgend(&input_message);
                    StoreGlobalXid(globalXidString);
#else
            ...
            case 'e':       /*start node of twophase transaction*/
                ...
                break;
            case 'R':            /* participants of twophase transaction */
                {
            ...
            case 'a':            /* logical apply : pgxc_node_send_apply */
                {
                    if (IS_PGXC_DATANODE && IsConnFromCoord())
                    {
                        ...
                        if (c == 'Y')
                        {
                        	logicl_apply_set_ignor_pk_conflict(true);
                        }
                        else if (c == 'N')
                        {
...
{
    return (IsNormalPostgres && false == IsPGXCMainCluster);
}
